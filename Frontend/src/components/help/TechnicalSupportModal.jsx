...
function fileIcon(type) {
  if (type.startsWith("image/"))
    return <FaFileImage className="text-[#730042]" size={13} />;

  if (type === "application/pdf")
    return <FaFilePdf className="text-[#730042]" size={13} />;
...
export default function TechnicalSupportModal({ role, onClose }) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        {/* Header */}
        <div
          className="px-6 pt-6 pb-4 flex items-start justify-between flex-shrink-0"
          style={{ background: "#FBEAF0" }}
        >
          <div className="flex items-center gap-3">
            <div
              className="w-10 h-10 rounded-full border border-[#F7C1C1] flex items-center justify-center flex-shrink-0"
            ...
            <div>
              ...
              <p className="text-[11px] text-[#993556]">
                We'll reply on your registered email
              </p>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            className="text-[#993556] hover:text-[#730042]"
          >
            <FaTimes size={14} />
          </button>
        </div>
        ...
        {sent ? (
          <div className="px-6 py-8 text-center">
            ...
            <p className="text-xs text-gray-500 mb-5">
              Our team will get back to you over email shortly.
            </p>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1.5">
                ...
                <span className="text-gray-400 font-normal">
                  (optional — screenshots, files, up to {MAX_FILES})
                </span>
              </label>
  ...
  onClick={() => {
    window.open(
      "https://tawk.to/chat/6a9ec28808f5a2345548cf24/1k1u2fq91?layout=modern",
      "_blank",
      "noopener,noreferrer"
    );
  }}
  className="flex-1 py-2.5 rounded-xl bg-[#730042] text-[12px] font-medium text-white hover:opacity-90 transition-opacity"
>
  Live Chat
</button>
