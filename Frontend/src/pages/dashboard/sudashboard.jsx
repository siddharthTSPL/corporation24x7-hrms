...
const validateForm = (form, isEdit) => {
  ...
  else if (!NAME_REGEX_NONEMPTY.test(form.l_name.trim()))
...
const AttendanceMap = ({ checkins = [], loading }) => {
  ...
  useEffect(() => {
    ...
    const bounds = [];
    checkins.forEach(
      ({ lat, lng, name, role, dept, email, checkIn, checkedOut }) => {
        if (!lat || !lng) return;
...
function SuperAdminDashboard() {
  ...
  const admins = Array.isArray(adminsData?.admins)
    ...
    : Array.isArray(adminsData)
      ? adminsData
      : [];
  const employees = Array.isArray(empData?.users)
    ? empData.users
  ...
  const stats = [
    ...
    },
    {
      ...
      value: deptLoading || empLoading ? "—" : totalEmpCount,
          ...
          {deptLoading ? (
            ...
            <div>
              {departments.map((dep) => (
                ...
                >
                  <p className="text-[11px] sm:text-[12px] font-semibold text-[#0d0209] w-12 sm:w-16 flex-shrink-0">
                    {dep.department}
                  </p>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    {isManager && (
                      <span className="text-[9px] bg-[#f7ecf3] text-[#730042] px-1.5 py-0.5 rounded-full font-bold uppercase">
                        MGR
                      </span>
                    )}
                    {role && (
