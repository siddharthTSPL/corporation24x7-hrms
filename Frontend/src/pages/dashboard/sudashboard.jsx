...
export const getDepartmentName = (dept) => DEPT_FULL_FORMS[dept] || dept || "—";
...
const validateForm = (form, isEdit) => {
  ...
  else if (!NAME_REGEX_NONEMPTY.test(form.f_name.trim()))
    e.f_name = "Only letters and spaces are allowed";

  if (form.l_name.trim() && !NAME_REGEX_NONEMPTY.test(form.l_name.trim()))
    e.l_name = "Only letters and spaces are allowed";

  if (!form.work_email.trim()) e.work_email = "Work email is required";
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.work_email))
    e.work_email = "Enter a valid email address";
  ...
  if (!form.personal_contact.trim())
    e.personal_contact = "Personal contact is required";
  else if (!/^[6-9]\d{9}$/.test(form.personal_contact))
    e.personal_contact = "Enter a valid 10-digit mobile number";

  if (!form.e_contact.trim()) e.e_contact = "Emergency contact is required";
  else if (!/^[6-9]\d{9}$/.test(form.e_contact))
    e.e_contact = "Enter a valid 10-digit mobile number";
  else if (form.personal_contact && form.e_contact === form.personal_contact)
    e.e_contact = "Emergency contact must be different from personal contact";
  ...
  if (
    !form.is_fresher &&
    (!form.total_experience || Number(form.total_experience) <= 0)
  )
    e.total_experience = "Enter total experience in years";

  if (
    form.aadhaar_number &&
    !/^\d{12}$/.test(form.aadhaar_number.replace(/\s/g, ""))
  )
  ...
  if (
    ...
    !NAME_REGEX_NONEMPTY.test(form.account_holder_name.trim())
  )
...
function SearchableSelect({
  ...
  return (
    <div className="relative" ref={wrapRef}>
      ...
      >
        <span
          className={`truncate ${selectedLabel ? "text-[#0d0209]" : "text-[#c499b4]"}`}
        >
          {selectedLabel || placeholder}
        </span>
        <FaAngleDown
          size={11}
          className={`text-[#c499b4] flex-shrink-0 ml-2 transition-transform ${open ? "rotate-180" : ""}`}
        />
      </button>
      {open && !disabled && (
        <div className="absolute z-20 mt-1 w-full bg-white border border-[#e8d5e2] rounded-lg shadow-lg max-h-52 flex flex-col overflow-hidden">
          ...
          <div className="overflow-y-auto">
            {filtered.length === 0 ? (
              <p className="px-3 py-2.5 text-[12px] text-[#c499b4]">
                No results
              </p>
            ) : (
              filtered.map((o) => (
...
function PermissionEditor({ permissions, onChange }) {
  const togglePerm = (module, key) => {
    onChange({
      ...permissions,
      [module]: { ...permissions[module], [key]: !permissions[module][key] },
    });
  };
  ...
  const toggleModule = (module) => {
    ...
    const next = Object.fromEntries(
      Object.keys(permissions[module]).map((k) => [k, !allOn]),
    );
    ...
    <div className="space-y-3">
      {Object.entries(PERMISSION_META).map(([module, meta]) => {
        ...
        return (
          <div
            key={module}
            className="border border-[#e8d5e2] rounded-xl overflow-hidden"
          >
            <div
            ...
            >
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-[13px] font-semibold text-[#0d0209] truncate">
                    {meta.label}
                  </p>
                  <p className="text-[10px] text-[#c499b4] mt-0.5">
                    {onCount}/{total} permissions enabled
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0 ml-2">
                {someOn && !allOn && (
                  <span className="hidden sm:inline text-[10px] bg-amber-50 text-amber-600 border border-amber-200 px-2 py-0.5 rounded-full font-semibold">
                    Partial
                  </span>
                )}
                {allOn && (
                  <span className="hidden sm:inline text-[10px] bg-emerald-50 text-emerald-600 border border-emerald-200 px-2 py-0.5 rounded-full font-semibold">
                    All On
                  </span>
                )}
            ...
            <div className="border-t border-[#f0dcea] divide-y divide-[#f7ecf3]">
              {Object.entries(meta.keys).map(([key, label]) => (
                <div
                  key={key}
                  className="flex items-center justify-between px-3 sm:px-5 py-2.5 hover:bg-[#fdf5f9] transition-colors"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <div
                      className={`w-1.5 h-1.5 rounded-full flex-shrink-0 transition-colors ${modulePerms[key] ? "bg-[#730042]" : "bg-gray-200"}`}
                    />
                    <span className="text-[12px] text-[#4a3040] truncate">
                      {label}
                    </span>
                  </div>
    ...
    <div className="fixed inset-0 z-[1100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-[rgba(13,2,9,0.75)] backdrop-blur-md">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl animate-[modalUp_0.22s_ease-out]">
        <div className="px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4 border-b border-[#e8d5e2] flex items-center justify-between">
          <div className="min-w-0 mr-3">
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <h2 className="text-sm sm:text-base font-bold text-[#0d0209] tracking-tight truncate">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl text-[#7a5568] hover:bg-[#f7ecf3] hover:text-[#730042] transition-colors flex-shrink-0 min-w-[36px] min-h-[36px] flex items-center justify-center"
          >
            <FaTimes size={13} />
          </button>
        </div>
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-5">
            ...
            <>
              <div className="flex items-start gap-3 p-3 sm:p-4 bg-amber-50 border border-amber-200 rounded-xl mb-4 sm:mb-5">
                ...
                <p className="text-[12px] sm:text-[13px] text-amber-800 leading-relaxed font-medium">
                  ...
                  Once the working status is changed, it cannot be modified
                  again.
                </p>
              </div>
              <p className="text-[12px] text-[#7a5568] mb-3 font-medium">
                Select the reason for status change:
              </p>
                ...
                {WORKING_STATUS_OPTIONS.map((opt) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span className="text-[13px] font-semibold text-[#0d0209]">
                        {opt.label}
                      </span>
                      {selectedStatus === opt.value && (
                        <FaCheck size={10} className="ml-auto text-[#730042]" />
                      )}
                    </button>
                  );
                })}
          ...
          {step === "noticeAsk" && (
            <>
              <p className="text-[13px] font-bold text-[#0d0209] mb-1">
              ...
              <p className="text-[12px] text-[#7a5568] mb-4 leading-relaxed">
                ...
                payroll continues) before marking <strong>{name}</strong> as{" "}
                <strong style={{ color: meta.color }}>{meta.label}</strong>, or{" "}
                <strong>No</strong> to mark them {meta.label} right away.
              </p>
          ...
          {step === "noticeDetails" && (
            <>
              <p className="text-[13px] font-bold text-[#0d0209] mb-3">
                Notice period details
              </p>
              <label className="block text-[11px] font-semibold text-[#7a5568] mb-1">
              ...
              <p className="text-[11px] text-[#7a5568] leading-relaxed">
                {name} stays <strong>Working</strong> and gets normal payroll
                till this date. On this date the system will automatically mark
                them <strong style={{ color: meta.color }}>{meta.label}</strong>{" "}
                and generate their Full &amp; Final settlement for that last
                month.
              </p>
            </>
          )}
          ...
          {step === 2 && (
            <>
              <div className="flex items-start gap-3 p-3 sm:p-4 bg-red-50 border border-red-200 rounded-xl mb-4 sm:mb-5">
                ...
                />
                <div>
                  <p className="text-[13px] text-red-800 font-bold mb-1">
                    Final Confirmation
                  </p>
                  <p className="text-[12px] text-red-700 leading-relaxed">
                    You are about to mark <strong>{name}</strong> as{" "}
              ...
              >
                <div
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${avaColor(admin.f_name || "")}, #cd166e)`,
                  }}
                >
                  {initials(name)}
                </div>
                <div className="min-w-0">
                  <p className="text-[13px] font-bold text-[#0d0209] truncate">
                    {name}
                  </p>
                  <p className="text-[11px] text-[#7a5568] truncate">
                    {admin.designation} · {getDepartmentName(admin.department)}
                  </p>
                  <span
                    className="inline-flex items-center mt-1 text-[10px] font-bold px-2 py-0.5 rounded-full"
                    style={{
                      color: meta.color,
                      background: meta.bg,
              ...
              {noticePeriodAllowed && (
                <p className="text-[11px] text-red-700 mt-2 font-medium">
                  Notice period: {noticePeriodMonths} month(s), last working day{" "}
                  {new Date(lastWorkingDay).toDateString()}.
                </p>
              )}
            </>
          )}
              ...
              step === 1
                ...
                : step === "noticeAsk"
                  ...
                  : step === "noticeDetails"
                    ...
                    : () =>
                        setStep(
                          noticePeriodAllowed ? "noticeDetails" : "noticeAsk",
                        )
            }
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[90vh] animate-[modalUp_0.22s_ease-out]">
        <div className="sticky top-0 z-10 bg-white px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4 border-b border-[#e8d5e2] flex items-center justify-between rounded-t-2xl">
          <div className="min-w-0 mr-3">
            ...
            <p className="text-[11px] text-[#c499b4] truncate">
              {name} · <span className="font-semibold">{userModel}</span> ·{" "}
              {user.designation}
            </p>
          </div>
          <button
        ...
        <div className="overflow-y-auto flex-1 px-4 sm:px-6 py-4 sm:py-5">
          {permLoading ? (
            ...
            <>
              <p className="text-[12px] text-[#7a5568] mb-4 leading-relaxed">
                Toggle individual permissions or use the module switch to grant
                all at once.
              </p>
              <PermissionEditor
...
function AdminModal({ open, onClose, initial, onSave, loading }) {
  ...
  const officeStateOptions = useMemo(
    () =>
      officeCountryIso
        ? State.getStatesOfCountry(officeCountryIso).map((s) => ({
            value: s.isoCode,
            label: s.name,
          }))
        : [],
  ...
  const officeCityOptions = useMemo(
    () =>
      officeCountryIso && officeStateIso
        ? City.getCitiesOfState(officeCountryIso, officeStateIso).map(
            (c) => c.name,
          )
        : [],
    [officeCountryIso, officeStateIso],
  ...
  const handleResidentialChange = (e) => {
    ...
    setForm(next);
    if (submitted || touched.residential_address)
      setErrors(validateForm(next, isEdit));
  };
  ...
  const handleSave = () => {
    ...
    const { confirmPassword, residential_address, permanent_address, ...rest } =
      form;
    const address = [
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl shadow-2xl flex flex-col max-h-[94vh] animate-[modalUp_0.22s_ease-out]">
        <div className="sticky top-0 z-10 bg-white px-4 sm:px-7 pt-4 sm:pt-5 pb-3 sm:pb-4 border-b border-[#e8d5e2] flex items-center justify-between rounded-t-2xl">
          <div className="min-w-0 mr-3">
            ...
            <p className="text-[11px] text-[#c499b4] mt-0.5">
              {isEdit
                ? "Update admin details and permissions"
                : "Fill in details and assign permissions"}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 px-4 sm:px-7 py-4 sm:py-5">
              ...
              <div>
                ...
                <div className="relative">
                  <FInput
                    ...
                    err={showErr("password")}
                    className="pr-11"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPass((v) => !v)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[#c499b4] hover:text-[#730042] transition-colors min-w-[28px] min-h-[28px] flex items-center justify-center"
                  >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-3 sm:mt-4">
            <div>
              ...
              <FInput
                ...
                onChange={setFiltered("personal_contact", DIGITS_ONLY_REGEX)}
                onBlur={blur("personal_contact")}
                err={showErr("personal_contact")}
                maxLength={10}
                inputMode="numeric"
              />
            ...
            <div>
              ...
              <FInput
                ...
                onChange={setFiltered("e_contact", DIGITS_ONLY_REGEX)}
                onBlur={blur("e_contact")}
                err={showErr("e_contact")}
                maxLength={10}
                inputMode="numeric"
              />
          ...
          <div className="mt-3 sm:mt-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                <FSel
                  value={officeStateIso}
                  onChange={handleOfficeStateChange}
                  disabled={
                    !officeCountryIso || officeStateOptions.length === 0
                  }
                >
                  <option value="">
                    {officeStateOptions.length
                      ? "Select state"
                      : "No states available"}
                  </option>
                  {officeStateOptions.map((s) => (
              ...
              <div>
                <FSel
                  ...
                  err={showErr("office_location")}
                  disabled={!officeStateIso || officeCityOptions.length === 0}
                >
                  <option value="">
                    {officeCityOptions.length
                      ? "Select city"
                      : "Select state first"}
                  </option>
                  {officeCityOptions.map((city) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div className="flex items-center gap-3 pt-4 sm:pt-5">
              <input
                ...
                onChange={set("is_fresher")}
                className="w-5 h-5 accent-[#730042] cursor-pointer rounded flex-shrink-0"
              />
              <label
                htmlFor="is_fresher"
                className="text-[11px] font-bold tracking-widest uppercase text-[#7a5568] cursor-pointer select-none"
              >
                Is Fresher
              </label>
            </div>
            <div>
              ...
              <FInput
                ...
                onChange={set("total_experience")}
                onBlur={blur("total_experience")}
                err={showErr("total_experience")}
                disabled={form.is_fresher}
                className={
                  form.is_fresher ? "opacity-40 cursor-not-allowed" : ""
                }
              />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-3 sm:mt-4">
              ...
              <div>
                ...
                <FInput
                  ...
                  onChange={set("previous_designation")}
                />
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              ...
              <FInput
                ...
                onChange={setFiltered("aadhaar_number", DIGITS_ONLY_REGEX)}
                onBlur={blur("aadhaar_number")}
                err={showErr("aadhaar_number")}
                maxLength={12}
                inputMode="numeric"
              />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-3 sm:mt-4">
            ...
            <div>
              ...
              <SearchableSelect
                ...
                options={cityOptions}
                placeholder={
                  cityOptions.length ? "Select city" : "Select a state first"
                }
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mt-3 sm:mt-4">
            <div className="col-span-2 sm:col-span-1">
              ...
              <FInput
                ...
                value={form.pincode}
                onChange={setFiltered("pincode", DIGITS_ONLY_REGEX)}
                onBlur={blur("pincode")}
                err={showErr("pincode")}
                maxLength={6}
                inputMode="numeric"
          ...
          <SecHead icon={<FaUniversity size={11} />}>
            Banking Details{" "}
            <span className="normal-case tracking-normal font-normal text-[#c499b4] ml-1">
              (optional)
            </span>
          </SecHead>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            ...
            <div>
              ...
              <FInput
                ...
                onChange={setFiltered("account_holder_name", NAME_REGEX)}
                onBlur={blur("account_holder_name")}
                err={showErr("account_holder_name")}
              />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-3 sm:mt-4">
            ...
            <div>
              ...
              <FInput
                ...
                onChange={setUpper("ifsc_code")}
                onBlur={blur("ifsc_code")}
                err={showErr("ifsc_code")}
                maxLength={11}
              />
        ...
        <div className="sticky bottom-0 bg-white border-t border-[#e8d5e2] px-4 sm:px-7 py-3 sm:py-4 flex flex-col xs:flex-row items-start xs:items-center gap-2 xs:gap-3 rounded-b-2xl">
          {submitted && hasErrors(errors) && (
            <p className="text-[11px] text-red-500 font-medium flex-1 w-full xs:w-auto">
          ...
          <div className="flex gap-2 sm:gap-3 w-full xs:w-auto xs:ml-auto flex-shrink-0">
            <button
              onClick={onClose}
              className="flex-1 xs:flex-none px-3 sm:px-4 py-2.5 rounded-xl border border-[#e8d5e2] text-[13px] font-medium text-[#7a5568] hover:border-[#730042] hover:text-[#730042] transition-colors whitespace-nowrap min-h-[44px]"
            >
              Cancel
            </button>
            <button
              onClick={handleSave}
              disabled={loading}
              className="flex-1 xs:flex-none flex items-center justify-center gap-2 px-3 sm:px-5 py-2.5 rounded-xl bg-[#730042] text-white text-[13px] font-semibold hover:bg-[#4a0029] active:scale-95 transition disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap min-h-[44px]"
            >
              <FaCheck size={10} />
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-[rgba(13,2,9,0.7)] backdrop-blur-md">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg shadow-2xl animate-[modalUp_0.22s_ease-out]">
        <div className="px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4 border-b border-[#e8d5e2] flex items-center justify-between">
          <h2 className="text-base sm:text-lg font-bold text-[#0d0209]">
            {initial ? "Edit Announcement" : "New Announcement"}
          </h2>
          <button
            onClick={onClose}
            className="p-2 rounded-xl text-[#7a5568] hover:bg-[#f7ecf3] hover:text-[#730042] transition-colors min-w-[36px] min-h-[36px] flex items-center justify-center"
          >
            <FaTimes size={13} />
          </button>
        </div>
        ...
        <div className="px-4 sm:px-6 pb-4 sm:pb-5 flex justify-end gap-2 sm:gap-3">
          <button
            onClick={onClose}
            className="px-3 sm:px-4 py-2.5 rounded-xl border border-[#e8d5e2] text-[13px] font-medium text-[#7a5568] hover:border-[#730042] hover:text-[#730042] transition-colors min-h-[44px]"
          >
            Cancel
          </button>
          <button
            ...
            className="flex items-center gap-2 px-4 sm:px-5 py-2.5 rounded-xl bg-[#730042] text-white text-[13px] font-semibold hover:bg-[#4a0029] transition disabled:opacity-50 disabled:cursor-not-allowed min-h-[44px]"
          >
            <FaCheck size={10} />
...
function ReviewModal({ open, onClose, admins, onSave, loading }) {
  ...
  useEffect(() => {
    if (open) setForm({ adminid: "", rating: 0, comment: "" });
  }, [open]);
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-[rgba(13,2,9,0.7)] backdrop-blur-md">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl animate-[modalUp_0.22s_ease-out]">
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-5 space-y-3 sm:space-y-4">
          <div>
            ...
            <FSel
              value={form.adminid}
              onChange={(e) =>
                setForm((f) => ({ ...f, adminid: e.target.value }))
              }
            >
              <option value="">Choose admin…</option>
              {admins.map((a) => (
                <option key={a._id} value={a._id}>
                  {a.f_name} {a.l_name} – {a.designation} (
            ...
            <div className="flex gap-1 sm:gap-2 mt-1">
              {[1, 2, 3, 4, 5].map((n) => (
                <button
                  ...
                  className={`text-2xl transition-transform hover:scale-110 min-w-[44px] min-h-[44px] flex items-center justify-center ${form.rating >= n ? "text-amber-400" : "text-gray-200"}`}
                >
                  ★
                </button>
              ))}
        ...
        <div className="px-4 sm:px-6 pb-4 sm:pb-5 flex justify-end gap-2 sm:gap-3">
          ...
          <button
            onClick={() => onSave(form)}
            disabled={loading || !form.adminid || !form.rating || !form.comment}
            className="flex items-center gap-2 px-4 sm:px-5 py-2.5 rounded-xl bg-[#730042] text-white text-[13px] font-semibold hover:bg-[#4a0029] transition disabled:opacity-50 disabled:cursor-not-allowed min-h-[44px]"
          >
            <FaStar size={10} />
...
function ConfirmModal({
  ...
  const iconWrap = isDanger
    ...
    : "bg-[#f7ecf3] border-[#e8d5e2] text-[#730042]";
  const confirmCls = isDanger
    ? "bg-red-600 hover:bg-red-700"
    : "bg-[#730042] hover:bg-[#4a0029]";

  return (
    <div
      className="fixed inset-0 z-[1200] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-[rgba(13,2,9,0.75)] backdrop-blur-md"
      onClick={onClose}
    >
      <div
        className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm shadow-2xl animate-[modalUp_0.22s_ease-out]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="px-4 sm:px-6 pt-5 pb-4 flex items-start gap-3">
          <div
            className={`w-10 h-10 rounded-full border flex items-center justify-center flex-shrink-0 ${iconWrap}`}
          >
            <FaExclamationTriangle size={15} />
          </div>
          <div className="min-w-0">
            <h2 className="text-[15px] font-bold text-[#0d0209] leading-tight">
              {title}
            </h2>
            {message && (
              <p className="text-[12px] sm:text-[13px] text-[#7a5568] mt-1.5 leading-relaxed">
                {message}
              </p>
            )}
          </div>
        ...
        <div className="px-4 sm:px-6 pb-4 sm:pb-5 flex gap-2 sm:gap-3">
          <button
            ...
            onClick={onClose}
            disabled={loading}
            className="flex-1 px-3 sm:px-4 py-2.5 rounded-xl border border-[#e8d5e2] text-[13px] font-medium text-[#7a5568] hover:border-[#730042] hover:text-[#730042] transition-colors min-h-[44px] disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            ...
            onClick={onConfirm}
            disabled={loading}
            className={`flex-1 flex items-center justify-center gap-2 px-3 sm:px-5 py-2.5 rounded-xl text-white text-[13px] font-semibold active:scale-95 transition disabled:opacity-50 disabled:cursor-not-allowed min-h-[44px] ${confirmCls}`}
          >
            <FaCheck size={10} />
...
const AttendanceMap = ({ checkins = [], loading }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      L.tileLayer(
        "https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}.png?key=cb1_2rmj_1_70182d7daf2ea5f395d53791",
        {
...
function StatCard({ icon, label, value, sub, color, bgColor, bar, badge }) {
  return (
    <div className="bg-white rounded-2xl border border-[#e8d5e2] shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-200 overflow-hidden relative p-4 sm:p-5">
      ...
      />
      <div
        className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center mb-3 sm:mb-4 text-sm sm:text-base"
        style={{ background: bgColor, color }}
      >
        {icon}
      </div>
      <p className="text-[9px] sm:text-[10px] font-bold tracking-[0.8px] uppercase text-[#7a5568] mb-1">
        {label}
      </p>
      <div className="flex items-end gap-2">
        <p className="text-2xl sm:text-3xl font-bold text-[#0d0209] leading-none mb-1">
          {value}
        </p>
        {badge && (
          <span
            className="mb-1.5 text-[9px] font-bold px-1.5 py-0.5 rounded-full"
            style={{
              color: badge.color,
              background: badge.bg,
      ...
      <p
        className="text-[10px] sm:text-[11px] font-medium mt-1.5"
        style={{ color }}
      >
        {sub}
      </p>
...
function AddAdminButton({ isAtLimit, onClick, variant = "header" }) {
  const base =
    "flex items-center justify-center gap-1.5 rounded-xl text-[12px] font-semibold transition-colors min-h-[44px] relative group";

  const variantCls = {
    header:
      "bg-[#730042] text-white px-2.5 sm:px-3 py-1.5 sm:py-2 hover:bg-[#4a0029]",
    hero: "bg-white/15 border border-white/25 text-white px-3 py-2 hover:bg-white/25 backdrop-blur-sm",
    heroMobile:
      "flex-1 bg-white/15 border border-white/25 text-white px-3 py-2.5 hover:bg-white/25 backdrop-blur-sm",
  };
  ...
  return (
    ...
    >
      <FaPlus size={9} />
      <span className={variant === "header" ? "hidden xs:inline" : ""}>
        Add Admin
      </span>
      {isAtLimit && (
...
function SuperAdminDashboard() {
  ...
  const [workingStatusModal, setWorkingStatusModal] = useState({
    open: false,
    admin: null,
  });
  ...
  const [demoteModal, setDemoteModal] = useState({ open: false, admin: null });

  useEffect(() => {
    const REFRESH_FLAG_KEY = "dashboardAutoRefreshed";
    const hasAlreadyRefreshedThisSession =
      sessionStorage.getItem(REFRESH_FLAG_KEY);
  ...
  const { data: activeUserData, isLoading: activeUserLoading } =
    useSuperAdminActiveUserCount();
  ...
  const { mutate: promoteAdminToSuperAdmin, isPending: promotingToSuperAdmin } =
    usePromoteAdminToSuperAdmin();
  // FIX: Admin -> Manager demote (previously wrongly used demoteSuperAdminToAdmin)
  const { mutate: demoteAdminToManager, isPending: demotingToManager } =
  ...
  const { mutate: updatePermissions, isPending: updatingPerms } =
    useUpdatePermissions();
  const { mutate: setAdminWorkingStatus, isPending: settingWorkingStatus } =
    useSetAdminWorkingStatus();
  ...
  const isActiveEmployee = (u) =>
    (u?.working_status || "working").toLowerCase() === "working";
  const employees = (
  ...
  const departments = Array.isArray(deptData?.departments)
    ? deptData.departments
    : [];
  const totalEmpCount = deptData?.totalEmployees ?? employees.length;
  const announcements = (
    Array.isArray(annRaw?.announcements)
  ...
  const adminLeaves = Array.isArray(leavesRaw?.adminLeaves?.leaves)
    ? leavesRaw.adminLeaves.leaves
    : [];
  ...
  const userUsagePercent =
    allowedUsers > 0
      ? Math.min(Math.round((activeUserCount / allowedUsers) * 100), 100)
      : 0;
  const isNearLimit = allowedUsers > 0 && activeUserCount >= allowedUsers * 0.8;
  const isAtLimit =
    allowedUsers > 0
      ? activeUserCount >= allowedUsers
      : (activeUserData?.is_limit_reached ?? false);
  ...
  const displayName =
    [superAdmin?.f_name, superAdmin?.l_name].filter(Boolean).join(" ") ||
    "there";
  ...
  const activeUserColor = isAtLimit
    ? "#d93025"
    : isNearLimit
      ? "#b8760a"
      : "#730042";
  ...
  const stats = [
    {
      ...
      sub: `${admins.filter((a) => a.status === "active").length} active`,
      color: "#730042",
      bgColor: "#f7ecf3",
      bar: null,
      badge: null,
    },
    {
      ...
      value: activeUserLoading
        ? "—"
        : (superAdmin?.active_user_count ?? activeUserCount),
    ...
    },
    {
      ...
      sub: `${attendanceRate}% · ${stillOnDuty} on duty`,
      color: "#0d9e6e",
      bgColor: "#e8f7f1",
    ...
    },
    {
      icon: <FaCalendarAlt />,
      label: "Admin Leaves",
      value: leaveLoading ? "—" : pendingAdminLeaves,
      sub: pendingAdminLeaves > 0 ? "Needs attention" : "All clear ✓",
      color: pendingAdminLeaves > 0 ? "#b8760a" : "#0d9e6e",
      bgColor: pendingAdminLeaves > 0 ? "#fff8e1" : "#e8f7f1",
      bar: null,
      badge: null,
    },
    {
    ...
    },
    {
      ...
      sub: activeUserLoading
        ? "Loading…"
        : isAtLimit
          ? "User limit reached"
          : isNearLimit
            ? "Approaching limit"
            : `${allowedUsers - activeUserCount} seats remaining`,
  ...
  const saveAdmin = (form) => {
    if (adminModal.editing) {
      updateAdmin(
        ...
        { onSuccess: () => setAdminModal({ open: false, editing: null }) },
      );
    } else {
      createAdmin(form, {
        onSuccess: () => setAdminModal({ open: false, editing: null }),
  ...
  const handleAcceptLeave = (leave) =>
    acceptLeave({ id: leave._id, leaveFor: "admin" });
  const handleRejectLeave = (leave) =>
    rejectLeave({ id: leave._id, leaveFor: "admin" });

  // Admin -> Manager demote: opens React confirm modal (no window.confirm)
  const handleDemoteToManager = (admin) => {
    setDemoteModal({ open: true, admin });
  };

  const confirmDemoteToManager = () => {
    if (!demoteModal.admin) return;
    demoteAdminToManager(demoteModal.admin._id, {
      onSuccess: () => setDemoteModal({ open: false, admin: null }),
      onError: () => setDemoteModal({ open: false, admin: null }),
    });
  };
  ...
  const savePermissions = (payload) => {
    updatePermissions(payload, {
      onSuccess: () => setPermModal({ open: false, user: null }),
    });
  };
    ...
    return (
      <div className="min-h-screen bg-[#fdf5f9] p-3 sm:p-5 lg:p-7 font-[system-ui,sans-serif] text-[#0d0209]">
        <div className="flex justify-between items-center mb-5 flex-wrap gap-3">
          <div>
            <h1
              className="text-xl sm:text-2xl font-bold m-0 tracking-tight"
              style={{ fontFamily: "'Lora',serif" }}
            >
              Analytics
            </h1>
            <p className="text-[12px] text-[#8a6f68] mt-0.5">
              Organisation-wide insights across attendance, leave, payroll &
              more
            </p>
          </div>
          <ViewToggle />
    ...
    <div className="min-h-screen bg-[#fdf5f9] p-3 sm:p-5 lg:p-7 font-[system-ui,sans-serif] text-[#0d0209]">
      ...
      <div className="relative bg-gradient-to-br from-[#2a0017] via-[#730042] to-[#cd166e] rounded-2xl p-4 sm:p-8 lg:p-10 mb-5 sm:mb-6 shadow-xl">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-3 sm:mb-4">
            ...
            <span className="text-[10px] font-bold tracking-[1.5px] uppercase text-white/70">
              Super Administrator
            </span>
          </div>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs sm:text-sm text-white/60 max-w-lg leading-relaxed mb-4 sm:mb-5">
                "{thought}"
              </p>
            </div>
            <div className="hidden sm:flex items-center gap-2 flex-shrink-0 mt-1">
              ...
              <button
                ...
                className="flex items-center gap-1.5 bg-white/15 border border-white/25 text-white px-3 py-2 rounded-xl text-[12px] font-semibold hover:bg-white/25 transition-colors backdrop-blur-sm min-h-[44px]"
              >
                <FaStar size={9} /> Review
              </button>
              <a
                href="https://torchxsuite.com/login"
                target="_blank"
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-0">
            <span className="bg-white/10 border border-white/20 backdrop-blur-sm rounded-full px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-[11px] text-white/90 font-medium">
              🏢 {orgName}
            </span>
            <span className="bg-white/10 border border-white/20 backdrop-blur-sm rounded-full px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-[11px] text-white/90 font-medium">
              👥 {superAdmin?.active_user_count ?? activeUserCount} Employees
            </span>
            {presentToday > 0 && (
              <span className="bg-white/10 border border-white/20 backdrop-blur-sm rounded-full px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-[11px] text-white/90 font-medium">
            ...
            {pendingAdminLeaves > 0 && (
              <span className="bg-white/10 border border-white/20 backdrop-blur-sm rounded-full px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-[11px] text-white/90 font-medium">
                📋 {pendingAdminLeaves} Pending
              </span>
            )}
            <span className="hidden md:inline-flex bg-white/10 border border-white/20 backdrop-blur-sm rounded-full px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-[11px] text-white/90 font-medium">
              📆 {today}
            </span>
          </div>
          ...
          <div className="flex sm:hidden gap-2 mt-3 items-center">
            ...
            <button
              onClick={() => setReviewModal(true)}
              className="flex-1 flex items-center justify-center gap-1.5 bg-white/15 border border-white/25 text-white px-3 py-2.5 rounded-xl text-[12px] font-semibold hover:bg-white/25 transition-colors backdrop-blur-sm min-h-[44px]"
            >
              <FaStar size={9} /> Review
            </button>
            <a
        ...
        <div className="bg-white rounded-2xl border border-[#e8d5e2] shadow-sm overflow-hidden">
          <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-[#e8d5e2] flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
              ...
              <span className="font-bold text-[13px] sm:text-[15px] text-[#0d0209] truncate">
                Live Attendance Map
              </span>
            </div>
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              ...
              <button
                ...
                style={{
                  background:
                    "linear-gradient(135deg, #730042 0%, #9B2554 100%)",
                }}
              >
          ...
          <div className="px-4 sm:px-5 py-2.5 sm:py-3 bg-[#f7ecf3] border-t border-[#e8d5e2] flex flex-wrap gap-3 sm:gap-4 items-center">
            <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-[11px] text-[#7a5568]">
              <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-[#730042] border-2 border-white shadow-sm flex-shrink-0" />
              Manager
            </div>
            <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-[11px] text-[#7a5568]">
              <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-[#a0005c] border-2 border-white shadow-sm flex-shrink-0" />
              Employee
            </div>
            <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-[11px] text-[#7a5568]">
              <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-gray-300 border-2 border-white shadow-sm opacity-60 flex-shrink-0" />
              Checked out
            </div>
            <span className="ml-auto text-[10px] sm:text-[11px] text-[#c499b4]">
        ...
        <div className="bg-white rounded-2xl border border-[#e8d5e2] shadow-sm overflow-hidden flex flex-col max-h-[420px]">
          <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-[#e8d5e2] flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2 sm:gap-2.5">
              ...
              <span className="font-bold text-[13px] sm:text-[15px] text-[#0d0209]">
                Announcements
              </span>
            </div>
            <button
              onClick={() => setAnnModal({ open: true, editing: null })}
              className="flex items-center gap-1 sm:gap-1.5 bg-[#730042] text-white px-2.5 sm:px-3 py-1.5 rounded-xl text-[11px] sm:text-[12px] font-semibold hover:bg-[#4a0029] transition-colors min-h-[36px]"
            >
              <FaPlus size={9} /> New
            </button>
          </div>
          {annLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 gap-2 text-[#c499b4] text-center px-4">
              ...
              <p className="text-[12px]">
                No announcements yet. Publish one to notify your team.
              </p>
            </div>
          ) : (
            <div className="divide-y divide-[#f7ecf3] overflow-y-auto flex-1 min-h-0">
              {announcements.map((ann) => {
                ...
                return (
                  <div
                    key={ann._id}
                    className="px-4 sm:px-5 py-3 sm:py-4 hover:bg-[#fdf5f9] transition-colors"
                  >
                    <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2 flex-wrap">
                      <span
                        className={`text-[9px] sm:text-[10px] font-bold tracking-wide uppercase px-2 sm:px-2.5 py-0.5 rounded-full ${priorityChipCls(priority)}`}
                      >
                        {priority.charAt(0).toUpperCase() + priority.slice(1)}
                      </span>
                      <span className="text-[9px] sm:text-[10px] font-semibold bg-gray-50 text-gray-500 border border-gray-200 px-2 sm:px-2.5 py-0.5 rounded-full uppercase">
                        {audience}
                      </span>
                    </div>
                    <p className="text-[12px] sm:text-[13px] font-semibold text-[#0d0209] mb-1">
                      {ann.title}
                    </p>
                    <p className="text-[11px] sm:text-[12px] text-[#7a5568] leading-relaxed line-clamp-2">
                      {ann.message}
                    </p>
                    <div className="flex gap-1.5 sm:gap-2 mt-2 sm:mt-3 pt-2 sm:pt-2.5 border-t border-[#f7ecf3]">
                      <button
                        ...
                        className="flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-[11px] text-[#c499b4] hover:text-[#730042] hover:bg-[#f7ecf3] px-1.5 sm:px-2 py-1 rounded-lg transition-colors font-medium min-h-[32px]"
                      >
                        <FaEdit size={9} /> Edit
                      </button>
                      <button
                        ...
                        className="flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-[11px] text-[#c499b4] hover:text-red-600 hover:bg-red-50 px-1.5 sm:px-2 py-1 rounded-lg transition-colors font-medium min-h-[32px]"
                      >
                        <FaTrash size={9} /> Delete
                      </button>
                      {ann.expiresAt && (
                        <span className="ml-auto text-[9px] sm:text-[10px] text-[#c499b4] self-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 mb-4 sm:mb-5">
        <div className="bg-white rounded-2xl border border-[#e8d5e2] shadow-sm overflow-hidden">
          <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-[#e8d5e2] flex items-center justify-between">
            ...
            <span className="text-[10px] sm:text-[11px] text-[#c499b4] font-semibold">
          ...
          {deptLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 gap-2 text-[#c499b4]">
              <FaChartBar size={24} />
              <p className="text-[12px]">No departments yet.</p>
            </div>
          ) : (
            <div>
              {departments.map((dep) => (
                <div
                  key={dep.department}
                  className="px-4 sm:px-5 py-2.5 sm:py-3 border-b border-[#f7ecf3] last:border-0 flex items-center gap-3 sm:gap-4"
                >
                  <p
                    className="text-[11px] sm:text-[12px] font-semibold text-[#0d0209] w-24 sm:w-32 flex-shrink-0 truncate"
                    title={getDepartmentName(dep.department)}
                  >
                    {getDepartmentName(dep.department)}
                  </p>
                  <div className="flex-1 h-2 bg-[#e8d5e2] rounded-full overflow-hidden">
                    <div
                      className="h-full rounded-full bg-gradient-to-r from-[#4a0029] to-[#cd166e] transition-all duration-1000"
                      style={{
                        width: `${Math.round((dep.lastNumber / maxDept) * 100)}%`,
                      }}
                    />
                  </div>
                  <p className="text-[12px] sm:text-[13px] font-bold text-[#730042] w-5 sm:w-6 text-right flex-shrink-0">
                    {dep.lastNumber}
                  </p>
                </div>
              ))}
        ...
        <div className="bg-white rounded-2xl border border-[#e8d5e2] shadow-sm overflow-hidden flex flex-col">
          <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-[#e8d5e2] flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
              <FaCalendarAlt
                size={12}
                className="text-[#730042] flex-shrink-0"
              />
              <span className="font-bold text-[13px] sm:text-[15px] text-[#0d0209] truncate">
            ...
            {pendingAdminLeaves > 0 && (
              <span className="text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200 px-2 sm:px-2.5 py-1 rounded-full flex-shrink-0">
                {pendingAdminLeaves} pending
              </span>
            )}
          </div>
          <div className="overflow-y-auto flex-1 max-h-[320px] sm:max-h-[380px]">
            {leaveLoading ? (
              ...
              activeLeaves.map((leave) => {
                ...
                const name =
                  [emp.f_name, emp.l_name].filter(Boolean).join(" ") || "Admin";
                ...
                return (
                  <div
                    key={leave._id}
                    className="px-4 sm:px-5 py-3 sm:py-3.5 border-b border-[#f7ecf3] last:border-0 hover:bg-[#fdf5f9] transition-colors flex items-start gap-2.5 sm:gap-3"
                  >
                    <div
                      className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center text-[10px] sm:text-[11px] font-bold text-white flex-shrink-0"
                      style={{ background: leaveTypeColor(type) }}
                    >
                      {initials(name)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] sm:text-[13px] font-semibold text-[#0d0209] truncate">
                        {name}
                      </p>
                      <p className="text-[10px] sm:text-[11px] text-[#7a5568] mt-0.5 leading-snug">
                        {type.toUpperCase()} · {fmtDate(from)}
                        {to && to !== from ? ` → ${fmtDate(to)}` : ""}
                        {emp.designation ? ` · ${emp.designation}` : ""}
                      </p>
                      {leave.reason && (
                        <p className="text-[10px] sm:text-[11px] text-[#c499b4] mt-1 italic line-clamp-1">
                          "{leave.reason}"
                        </p>
                      )}
                      {pending ? (
                        ...
                        <span
                          className={`inline-flex items-center mt-2 text-[10px] font-bold px-2.5 py-0.5 rounded-full ${leaveStatusClass(leave.status)}`}
                        >
      ...
      <div className="bg-white rounded-2xl border border-[#e8d5e2] shadow-sm overflow-hidden mb-4 sm:mb-5">
        <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-[#e8d5e2] flex items-center justify-between flex-wrap gap-2 sm:gap-3">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <button
              onClick={() => setReviewModal(true)}
              className="flex items-center gap-1 sm:gap-1.5 border border-[#e8d5e2] text-[#7a5568] px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-[11px] sm:text-[12px] font-semibold hover:border-[#730042] hover:text-[#730042] transition-colors min-h-[36px] sm:min-h-[40px]"
            >
              <FaStar size={9} />{" "}
              <span className="hidden xs:inline">Review Admin</span>
        ...
        {adminsLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 gap-2 text-[#c499b4]">
            ...
            <p className="text-[13px] text-center px-4">
              No admins yet. Create one to delegate management.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 p-3 sm:p-5">
            {admins.map((admin) => {
              ...
              const workingStatus = (
                admin.working_status || "working"
              ).toLowerCase();
              const isNonWorking = workingStatus !== "working";
              const wsMeta =
                WORKING_STATUS_META[workingStatus] ||
                WORKING_STATUS_META.working;
              return (
                <div
                  key={admin._id}
                  className={`border rounded-xl p-3 sm:p-4 flex flex-col hover:shadow-md hover:-translate-y-1 transition-all duration-200 relative ${isNonWorking ? "border-red-200 bg-red-50/40" : "border-[#e8d5e2] hover:bg-[#fdf5f9]"}`}
                >
                  {isNonWorking && (
                    <div className="absolute top-2 right-2">
                      <span
                        ...
                        style={{
                          color: wsMeta.color,
                          background: wsMeta.bg,
                          border: `1px solid ${wsMeta.border}`,
                        }}
                      >
                        {wsMeta.label}
                      </span>
                  ...
                  <div
                    className={`w-10 h-10 sm:w-11 sm:h-11 rounded-full flex items-center justify-center text-[13px] sm:text-[14px] font-bold text-white mx-auto mb-2 sm:mb-3 ${isNonWorking ? "opacity-60" : ""}`}
                    style={{
                      background: `linear-gradient(135deg, ${avaColor(admin.f_name || "")}, #cd166e)`,
                    }}
                  >
                  ...
                  <p
                    className={`text-[12px] sm:text-[13px] font-bold text-center truncate ${isNonWorking ? "text-gray-400" : "text-[#0d0209]"}`}
                  >
                    {name}
                  </p>
                  <p className="text-[10px] sm:text-[11px] text-[#7a5568] text-center mt-0.5 truncate">
                    {admin.designation}
                  </p>
                  <p className="text-[10px] text-[#c499b4] text-center mt-0.5 truncate hidden sm:block">
                    {admin.work_email}
                  </p>
                  <div className="flex flex-wrap justify-center gap-1 mt-2 sm:mt-2.5">
                    <span
                      className={`text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full ${statusKey === "active" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : statusKey === "suspended" ? "bg-red-50 text-red-700 border border-red-200" : "bg-gray-50 text-gray-600 border border-gray-200"}`}
                    >
                      {statusKey.charAt(0).toUpperCase() + statusKey.slice(1)}
                    </span>
                    <span
                      className={`text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full ${roleBadgeCls(roleKey)}`}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-1 mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-[#f0dcea]">
                    {!isNonWorking ? (
                      <button
                        onClick={() =>
                          setWorkingStatusModal({ open: true, admin })
                        }
                    ...
                    {/* FIX: Admin -> Manager demote (uses useDemoteAdminToManager) */}
                    {roleKey !== "super_admin" && !isNonWorking && (
                      <button
                        onClick={() => handleDemoteToManager(admin)}
                        disabled={demotingToManager}
                        className="flex items-center justify-center gap-1 px-1.5 py-1.5 rounded-lg text-[10px] sm:text-[11px] font-medium text-[#7a5568] hover:bg-gray-50 hover:text-gray-700 transition-colors min-h-[34px] disabled:opacity-50"
                        title="Demote from Admin to Manager"
                      >
                        <FaUserSlash size={9} />{" "}
                        {demotingToManager ? "…" : "Demote"}
                      </button>
                    )}
                    <button
      ...
      <div className="bg-white rounded-2xl border border-[#e8d5e2] shadow-sm overflow-hidden mb-4 sm:mb-5">
        <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-[#e8d5e2] flex flex-col xs:flex-row xs:items-center gap-2 xs:gap-3">
          ...
          <div className="flex items-center gap-2 xs:ml-auto">
            <div className="relative flex-1 xs:flex-none">
              <FaSearch
                className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#c499b4]"
                size={10}
              />
              <input
            ...
            {filteredEmp.length > 10 && (
              <button
                ...
                className="flex items-center gap-1 sm:gap-1.5 border border-[#e8d5e2] text-[#7a5568] px-2.5 sm:px-3 py-2 rounded-xl text-[11px] sm:text-[12px] font-semibold hover:border-[#730042] hover:text-[#730042] transition-colors whitespace-nowrap min-h-[36px] sm:min-h-[40px]"
              >
                {empExpand ? "Less" : `All (${filteredEmp.length})`}
                <FaChevronRight
                  size={8}
                  className={`transition-transform ${empExpand ? "rotate-90" : ""}`}
                />
              </button>
            )}
          </div>
        ...
        {empLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 gap-2 text-[#c499b4]">
            ...
            <p className="text-[13px]">
              {empSearch ? "No matching employees." : "No employees found."}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2 sm:gap-3 p-3 sm:p-5">
            {displayEmp.map((emp, i) => {
              ...
              const isManager =
                roleKey === "manager" || roleKey === "senior_manager";
              return (
                <div
                  key={emp._id || i}
                  className="border border-[#e8d5e2] rounded-xl p-2.5 sm:p-3 flex items-center gap-2.5 sm:gap-3 hover:shadow-sm hover:bg-[#fdf5f9] transition-all duration-150"
                >
                  <div
                    className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center text-[11px] sm:text-[12px] font-bold text-white flex-shrink-0"
                    style={{
                      background: isManager
                        ? "linear-gradient(135deg,#730042,#cd166e)"
                        : `linear-gradient(135deg,${avaColor(emp.f_name || "")},${avaColor(emp.l_name || "A")})`,
                    }}
                  >
                    {initials(name)}
                  </div>
                  <div className="min-w-0">
                    <p className="text-[11px] sm:text-[12px] font-semibold text-[#0d0209] truncate leading-tight">
                      {name}
                    </p>
                    {roleLabel && (
                    ...
                    {designation && (
                      <p className="text-[10px] sm:text-[11px] text-[#7a5568] truncate mt-0.5">
                        {designation}
                      </p>
                    )}
                    {dept && (
                      <span className="text-[9px] sm:text-[10px] bg-[#f7ecf3] text-[#730042] px-1.5 py-0.5 rounded-full font-semibold inline-block mt-0.5 sm:mt-1">
        ...
        {!empExpand && filteredEmp.length > 10 && (
          <div className="px-4 sm:px-5 py-3 border-t border-[#f0dcea] text-center">
            <button
              onClick={() => setEmpExpand(true)}
              className="text-[12px] font-semibold text-[#730042] hover:text-[#4a0029] transition-colors flex items-center gap-1.5 mx-auto min-h-[44px]"
            >
      ...
      <AnnModal
        ...
        initial={
          annModal.editing
            ? {
                title: annModal.editing.title,
                message: annModal.editing.message,
                audience: annModal.editing.audience || "all",
                priority: annModal.editing.priority || "normal",
              }
            : null
        }
        onSave={saveAnn}
      ...
      <ConfirmModal
        ...
        onConfirm={confirmDemoteToManager}
        loading={demotingToManager}
        variant="warning"
        title="Demote to Manager"
        confirmLabel="Yes, Demote"
        message={
          demoteModal.admin
            ? `Demote ${[demoteModal.admin.f_name, demoteModal.admin.l_name].filter(Boolean).join(" ")} from Admin to Manager? Admin permissions will be removed.`
            : ""
        }
      />
    </div>
