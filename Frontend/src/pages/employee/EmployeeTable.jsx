...
import {
  ...
  useEditEmployee,
  useEditManager,
  usePromoteEmployeeToManager,
  usePromoteEmployeeToAdmin,
  usePromoteManagerToAdmin,
  useDemoteManagerToEmployee,
  useDemoteAdminToManager,
  useDemoteAdminToEmployee,
  useGetParticularEmployee,
  useGetParticularManager,
  useSetEmployeeWorkingStatus,
  useSetManagerWorkingStatus,
  useAdminInactiveUsers,
  useGetActiveUserCount,
...
export const DEPT_OPTIONS = ["OPR", "BPO", "ENG", "HR", "MGMT"];
...
function exportToCSV(data) {
  ...
  const buildRow = (u) => {
    ...
    return [
      ...
      u.e_contact ?? "",
      u.role ?? "",
      DEPT_FULL_FORMS[u.department] ?? u.department ?? "",
      u.designation ?? "",
      u.office_location ?? "",
      reportingPerson
        ? `${reportingPerson.f_name ?? ""} ${reportingPerson.l_name ?? ""}`.trim()
        : "",
      u.is_fresher ? "Yes" : "No",
      u.total_experience ?? "",
      u.previous_company ?? "",
      u.previous_designation ?? "",
      u.address ?? "",
      u.city ?? "",
      u.state ?? "",
      u.pincode ?? "",
  ...
  const getType = (u) =>
    u.type ||
    (u.role === "manager" || u.role === "senior_manager"
      ? "manager"
      : "employee");
  ...
  return (
    <div className={`flex flex-col gap-1 ${span2 ? "col-span-2" : ""}`}>
      <label className="text-[11px] font-semibold uppercase tracking-wider text-[#993556]">
        {label}
        {required && <span className="text-[#CD166E] ml-0.5">*</span>}
      </label>
      {children}
      {error && (
        <span className="text-[11px] text-[#A32D2D] flex items-center gap-1">
          ⚠ {error}
        </span>
      )}
    </div>
...
function FileUploadField({
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      ...
      if (!url) {
        console.warn(
          "Upload response did not contain a recognizable URL field:",
          res,
        );
      ...
      {value ? (
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg border border-[#F4C0D1] bg-[#F9F8F2]">
          ...
          <a
            ...
            className="text-sm text-[#730042] truncate flex-1 hover:underline"
          >
            {fileName || "View file"}
          </a>
          <button
            ...
            className="text-[#993556] hover:text-[#CD166E] flex-shrink-0"
          >
            <FaTimes size={12} />
          </button>
        </div>
...
function OfficeLocationFields({ form, onChange, errors }) {
  const states = form.office_location_country
    ? State.getStatesOfCountry(form.office_location_country)
    : [];
  const cities =
    form.office_location_country && form.office_location_state
      ? City.getCitiesOfState(
          form.office_location_country,
    ...
    <>
      <Field label="Country" required error={errors.office_location_country}>
        <select
          ...
          value={form.office_location_country}
          onChange={onChange}
          className={inputCls}
        >
      ...
      >
        ...
        >
          <option value="">
            {form.office_location_state ? "Select City" : "Select state first"}
          </option>
          {cities.map((c) => (
            <option
...
function AddressFields({ form, onChange, errors }) {
  ...
  const permStates = form.permanent_country
    ? State.getStatesOfCountry(form.permanent_country)
    : [];

  const resStateIso = resStates.find((s) => s.name === form.state)?.isoCode;
  const permStateIso = permStates.find(
    (s) => s.name === form.permanent_state,
  )?.isoCode;

  const resCities =
    form.country && resStateIso
      ? City.getCitiesOfState(form.country, resStateIso)
      : [];
  const permCities =
    form.permanent_country && permStateIso
      ? City.getCitiesOfState(form.permanent_country, permStateIso)
      : [];
...
function Badge({ label, type = "dept" }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${styles[type] ?? styles.dept}`}
    >
      {label}
    </span>
  );
  ...
  return (
    <div className="flex items-start gap-2.5 py-2 border-b border-[#F4C0D1]/50 last:border-0">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-[#993556]">
          {label}
        </p>
        <p className="text-sm text-[#730042] font-medium break-words">
          {value}
        </p>
      </div>
    </div>
...
function PermissionToggle({ label, value, onChange, disabled }) {
  return (
    <div className="flex items-center justify-between py-1.5 gap-2">
      <span
        className={`text-xs font-medium ${disabled ? "text-[#993556]/40" : "text-[#730042]"}`}
      >
        {label}
      </span>
      <button
        ...
        title={
          disabled ? "This permission cannot be changed for this role" : ""
        }
      >
        {value ? (
          <FaToggleOn
            size={22}
            className={disabled ? "text-[#F4C0D1]" : "text-[#CD166E]"}
          />
        ) : (
...
function getDisabledKeys(roleType) {
  if (roleType === "employee") {
    return {
      ...
      recruitment: [
        "can_view_hiring_requisitions",
        "can_create_hiring_requisition",
        "can_view_candidates",
        "can_add_candidate",
      ],
    };
  ...
  return (
    <div className="col-span-1 sm:col-span-2 space-y-3">
      {sections.map((sec) => (
        ...
        >
          <div
            className="px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-[#993556]"
            style={{ background: "#FBEAF0" }}
          >
            {sec.label}
          </div>
...
function ReportingManagerSelect({
  ...
  const managersList = (managersOnly?.managers ?? []).filter(
    (m) => m._id !== excludeId,
  );
  const withAdminList = (managersWithAdmin?.managers ?? []).filter(
    (m) => m._id !== excludeId,
  );
          ...
          <optgroup label="Admins">
            {adminList.map((a) => (
              <option key={a._id} value={a._id}>
                {a.f_name} {a.l_name} — {a.work_email} (
          ...
          <optgroup label="Managers">
            {pureManagers.map((m) => (
              <option key={m._id} value={m._id}>
                {m.f_name} {m.l_name} — {m.work_email} (
                {m.role?.replace("_", " ") || "Manager"})
              </option>
            ))}
          </optgroup>
        )}
      ...
      {open && (
        <div className="absolute z-20 mt-1 w-full max-h-56 overflow-y-auto bg-white border border-[#F4C0D1] rounded-xl shadow-xl py-1">
          ...
          {filtered.length > 0 ? (
            ...
            <div className="px-3 py-2 text-xs text-[#993556]/70">
...
function PermissionsDrawer({ userId, userModel, userRole, onClose }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.post("permission/assign/admin", {
        user_id: userId,
  ...
  return (
    ...
    >
      ...
      <div className="w-full sm:max-w-xs bg-white shadow-2xl flex flex-col border-l border-[#F4C0D1] h-full overflow-hidden">
        <div
          className="flex items-center justify-between px-4 py-3 border-b border-[#F4C0D1] flex-shrink-0"
          style={{ background: "#F9F8F2" }}
        >
          <div className="flex items-center gap-2">
            <FaKey size={12} className="text-[#CD166E]" />
            <p className="text-sm font-bold text-[#730042]">
              Manage Permissions
            </p>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-lg flex items-center justify-center text-[#993556] hover:bg-[#FBEAF0] flex-shrink-0"
          >
            <FaTimes size={12} />
          </button>
        </div>
        {loading ? (
          ...
          <>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              <div className="flex gap-2 mb-3">
                <button
                  ...
                  className="flex-1 py-1.5 rounded-lg text-xs font-semibold border border-[#F4C0D1] text-[#730042] hover:bg-[#FBEAF0]"
                >
                  Set All On
                </button>
                <button
            ...
            <div className="px-4 py-3 border-t border-[#F4C0D1] flex-shrink-0 bg-[#F9F8F2]">
              {msg && (
                <p
                  className={`text-xs mb-2 text-center font-medium ${msg.includes("success") ? "text-[#22C55E]" : "text-[#A32D2D]"}`}
                >
                  {msg}
                </p>
              )}
              <button
                onClick={handleSave}
                disabled={saving}
                className="w-full py-2.5 rounded-xl text-white text-sm font-semibold disabled:opacity-60 hover:opacity-90"
    ...
    >
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl p-5 sm:p-6 max-h-[92vh] overflow-y-auto animate-[modalPop_.2s_ease-out]">
        ...
        <p className="text-[12px] font-semibold text-[#993556] mb-3">
          {pending_asset_count} asset{pending_asset_count !== 1 ? "s" : ""}{" "}
          still assigned:
        </p>
        ...
        <div className="space-y-2 mb-4 max-h-56 overflow-y-auto">
          {assets.map((a) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[12px] font-semibold text-[#730042] truncate">
                  {a.asset_name}
                </p>
                <p className="text-[10px] text-[#993556] font-mono">
                  {a.asset_id}
                </p>
        ...
        <p className="text-[11px] text-[#993556] mb-4 leading-relaxed">
          Please go to <strong>Asset Management</strong> and revoke the asset(s)
          listed above before changing this person's employment status.
        </p>
...
function WorkingStatusSelector({
  currentStatus,
  onSave,
  loading,
  blockedInfo,
  ...
  return (
    <div className="mt-3 p-3 rounded-xl border border-[#F4C0D1] bg-[#F9F8F2]">
      <p className="text-[10px] font-bold uppercase tracking-wider text-[#993556] mb-2">
        Employment Status
      </p>
        ...
        <div className="mb-3 rounded-xl border border-[#FCA5A5] bg-[#FFF5F5] p-3">
          ...
          <div className="space-y-1.5 mb-2">
            {blockedInfo.assets.map((a) => (
              <div
                key={a._id}
                className="flex items-center gap-2 p-2 rounded-lg bg-white border border-[#FCA5A5]"
              >
          ...
          <button
            ...
            className="text-[11px] font-semibold text-[#730042] hover:underline"
          >
            Dismiss
          </button>
        </div>
      ...
      {step === "noticeAsk" ? (
        <div className="rounded-xl border border-[#F4C0D1] bg-white p-3">
          <p className="text-xs font-bold text-[#730042] mb-1">
            Does your company allow a notice period?
          </p>
          <p className="text-[11px] text-[#993556] mb-3 leading-relaxed">
            Choose <strong>Yes</strong> to run a notice period first (normal
            payroll continues) before marking this person{" "}
            <strong>{statusLabel}</strong>, or <strong>No</strong> to mark them{" "}
            {statusLabel} right away.
          </p>
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex-1 py-2 rounded-lg text-white text-xs font-bold transition-all"
              style={{ background: "#730042" }}
            >
              Yes, notice period
            </button>
        ...
        <div className="rounded-xl border border-[#F4C0D1] bg-white p-3">
          <p className="text-xs font-bold text-[#730042] mb-3">
            Notice period details
          </p>
          <label className="block text-[11px] font-semibold text-[#993556] mb-1">
          ...
          />
          <label className="block text-[11px] font-semibold text-[#993556] mb-1">
            Last working day (notice period end date)
          </label>
          <input
            type="date"
          ...
          <p className="text-[10px] text-[#993556] mb-3 leading-relaxed">
            Employee stays <strong>Working</strong> and gets normal payroll till
            this date. On this date the system will automatically mark them{" "}
            <strong>{statusLabel}</strong> and generate their Full &amp; Final
            settlement for that last month.
          </p>
          <div className="flex gap-2">
            <button
              onClick={() => setStep("noticeAsk")}
              className="flex-1 py-2 rounded-lg text-xs font-semibold border border-[#F4C0D1] text-[#730042] hover:bg-[#FBEAF0] transition-all"
            >
              Back
            </button>
          ...
          <div className="flex items-start gap-2 mb-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#7F1D1D] mt-0.5 leading-relaxed">
                {noticePeriodAllowed ? (
                  <>
                    A <strong>{noticePeriodMonths}-month</strong> notice period
                    will start now. On{" "}
                    <strong>{new Date(lastWorkingDay).toDateString()}</strong>{" "}
                    the employee will automatically be marked{" "}
                    <strong>{statusLabel}</strong> and their F&amp;F will be
                    generated.
                  </>
                ) : (
                  <>
                    Setting status to <strong>{statusLabel}</strong> is
                    permanent. The employee will be marked as inactive and
                    cannot be set back to <strong>Working</strong>.
                  </>
                )}
              </p>
            </div>
          ...
          {isAlreadyIrreversible ? (
            <div className="flex items-center gap-1.5 px-2 py-2 rounded-lg bg-[#F3F4F6] border border-[#E5E7EB]">
              ...
              <p className="text-[11px] text-[#6B7280]">
                Status is permanently set to <strong>{currentStatus}</strong>{" "}
            ...
            <>
              ...
              <button
                ...
                style={{
                  background:
                    isIrreversible && !noChange ? "#DC2626" : "#730042",
                }}
              >
...
function AccountSummaryDrawer({
  ...
  const isAdmin =
    ...
    userRole === "official";
  const empQuery = useGetParticularEmployee(
    !isManager && !isAdmin ? userId : null,
  );
  ...
  const leaveBalance = Array.isArray(rawLeaveBalance)
    ? rawLeaveBalance[0]
    : rawLeaveBalance;
    ...
    ? [
        {
          ...
          remaining:
            ...
            Math.max(
              0,
              Number(leaveBalance.EL?.accrued ?? 0) -
                Number(leaveBalance.EL?.availed ?? 0),
            ),
        },
        {
          label: "Sick Leave",
          ...
          remaining:
            ...
            Math.max(
              0,
              Number(leaveBalance.SL?.entitled ?? 0) -
                Number(leaveBalance.SL?.availed ?? 0),
            ),
        },
        ...(showML
          ? [
              {
                ...
                total: leaveBalance.ML || 0,
                used: 0,
                remaining: leaveBalance.ML || 0,
              },
            ]
          : []),
        ...(showPL
          ? [
              {
                ...
                total: leaveBalance.PL || 0,
                used: 0,
                remaining: leaveBalance.PL || 0,
              },
            ]
          : []),
        {
          label: "Paid by Company",
          total: leaveBalance.pbc || 0,
          used: 0,
          remaining: leaveBalance.pbc || 0,
        },
        {
          label: "Leave Without Pay",
          total: leaveBalance.lwp || 0,
          used: 0,
          remaining: leaveBalance.lwp || 0,
        },
      ]
    : [];
  ...
  const avgRating = reviews.length
    ? reviews.reduce((s, r) => s + (r.overallScore || 0) / 2, 0) /
      reviews.length
    : null;
  ...
  const roleBadgeEl = (r) => {
    ...
    if (r === "senior_manager")
      return <Badge label="Sr. Manager" type="smgr" />;
    if (r === "admin" || r === "senior_admin")
      return <Badge label="Admin" type="admin" />;
      ...
      >
        ...
        <div className="w-full sm:max-w-sm bg-white shadow-2xl flex flex-col border-l border-[#F4C0D1] h-full overflow-hidden">
          ...
          {loading ? (
              ...
              <div className="px-4 pt-4 pb-3 border-b border-[#F4C0D1] flex-shrink-0">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-[#730042] text-base leading-tight">
                      {person.f_name} {person.l_name}
                    </p>
                    <p className="text-xs text-[#993556] truncate mt-0.5">
                      {person.work_email}
                    </p>
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      ...
                      {person.department && (
                        <Badge
                          label={
                            DEPT_FULL_FORMS[person.department] ||
                            person.department
                          }
                          type="dept"
                        />
                    ...
                    {person.empid && (
                      <p className="text-[11px] text-[#993556] mt-1.5 font-mono bg-[#F9F8F2] px-1.5 py-0.5 rounded inline-block border border-[#F4C0D1]">
                        {person.empid}
                      </p>
                    )}
                  </div>
                ...
                {avgRating !== null && (
                  <div className="mt-3 flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-[#993556] font-medium">
                ...
                {tab === "info" && (
                  <div className="space-y-0.5">
                    ...
                    <InfoRow
                      ...
                      value={person.personal_contact}
                    />
                    <InfoRow
                      icon={<FaPhone size={10} />}
                      label="Emergency Contact"
                      value={person.e_contact}
                    />
                    <InfoRow
                    ...
                    <InfoRow
                      ...
                      label="Department"
                      value={
                        DEPT_FULL_FORMS[person.department] || person.department
                      }
                    />
                    <InfoRow
                    ...
                    <InfoRow
                      ...
                      value={
                        formatDate(
                          person.date_of_joining || person.createdAt,
                        ) || "—"
                      }
                    />
                    <InfoRow
                    ...
                    <InfoRow
                      ...
                      value={
                        person.marital_status
                          ? person.marital_status.charAt(0).toUpperCase() +
                            person.marital_status.slice(1)
                          : null
                      }
                    />
                    {person.address && (
                    ...
                    {person.bank_name && (
                      <div className="mt-3 pt-3 border-t border-[#F4C0D1]/50">
                        ...
                        <InfoRow
                          ...
                          label="Bank"
                          value={person.bank_name}
                        />
                        <InfoRow
                          icon={<FaUniversity size={10} />}
                          label="Account Holder"
                          value={person.account_holder_name}
                        />
                        <InfoRow
                          icon={<FaUniversity size={10} />}
                          label="Account No."
                      ...
                      <div className="mt-3 pt-3 border-t border-[#F4C0D1]/50">
                        ...
                        {person.resume && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 py-1.5 text-xs text-[#730042] hover:text-[#CD166E]"
                          >
                            <FaFileAlt size={10} /> Resume
                          </a>
                        )}
                        {person.aadhaar_card && (
                          <a
                            href={person.aadhaar_card}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 py-1.5 text-xs text-[#730042] hover:text-[#CD166E]"
                          >
                            <FaFileAlt size={10} /> Aadhaar Card
                          </a>
                        )}
                        {person.pan_card && (
                          <a
                            href={person.pan_card}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 py-1.5 text-xs text-[#730042] hover:text-[#CD166E]"
                          >
                            <FaFileAlt size={10} /> PAN Card
                          </a>
                        )}
                        {person.experience_letter && (
                          <a
                            href={person.experience_letter}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 py-1.5 text-xs text-[#730042] hover:text-[#CD166E]"
                          >
                            <FaFileAlt size={10} /> Experience Letter
                          </a>
                        )}
                      </div>
                    )}
                    {(person.Under_manager || person.reporting_manager) && (
                      <div className="mt-3 pt-3 border-t border-[#F4C0D1]/50">
                        ...
                        <div className="flex items-center gap-2 p-2 rounded-lg bg-[#FBEAF0] border border-[#F4C0D1]">
                          <Avatar
                            name={`${(person.Under_manager || person.reporting_manager)?.f_name ?? ""} ${(person.Under_manager || person.reporting_manager)?.l_name ?? ""}`}
                            size="sm"
                          />
                          <div className="min-w-0">
                            <p className="text-xs font-semibold text-[#730042] truncate">
                              {
                                (
                                  person.Under_manager ||
                            ...
                            <p className="text-[10px] text-[#993556] truncate">
                              {
                                (
                                  person.Under_manager ||
                                  person.reporting_manager
                    ...
                    {leaveCards.length > 0 ? (
                      <div className="grid grid-cols-2 gap-2">
                        {leaveCards.map((c) => (
                          <div
                            key={c.label}
                            className="p-3 rounded-xl border border-[#F4C0D1] bg-[#FBEAF0]"
                          >
                            <p className="text-[10px] font-bold uppercase tracking-wider text-[#993556]">
                              {c.label}
                            </p>
                            <p className="text-xl font-bold text-[#730042] mt-0.5">
                              {c.remaining}
                            </p>
                            <p className="text-[10px] text-[#993556] mt-1">
                              {c.used} used / {c.total} total
                            </p>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="text-center py-8 text-[#993556] text-sm">
                      ...
                      reviews.map((r, i) => (
                        ...
                        >
                          <div className="flex items-center justify-between mb-1.5 gap-2 flex-wrap">
                            <p className="text-xs font-semibold text-[#730042]">
                              {r.reviewer?.f_name} {r.reviewer?.l_name}
                            </p>
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-bold text-[#730042]">
                                {r.overallScore != null
                          ...
                          {(r.taskSubmission ||
                            ...
                            <div className="flex flex-wrap gap-2 mb-1 text-[10px] text-[#993556]/80">
                              {r.taskSubmission && (
                                <span>
                                  Task: {r.taskSubmission.percentage}% (
                                  {r.taskSubmission.rating})
                                </span>
                              )}
                              {r.behaviourEthics && (
                                <span>
                                  Behaviour: {r.behaviourEthics.score}/10
                                </span>
                              )}
                              {r.attendance && (
                                <span>
                                  Attendance: {r.attendance.percentage}%
                                </span>
                              )}
                            </div>
                          )}
                          <p className="text-xs text-[#993556]">{r.comment}</p>
                          <p className="text-[10px] text-[#993556]/60 mt-1">
                            {r.monthYear}
                          </p>
                        </div>
                      ))
              ...
              <div className="px-4 py-3 border-t border-[#F4C0D1] flex-shrink-0 bg-[#F9F8F2]">
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 py-2 rounded-lg border border-[#F4C0D1] text-xs font-semibold text-[#730042] hover:bg-[#FBEAF0]"
                  >
                    <FaEdit size={10} /> Edit
                  </button>
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 py-2 rounded-lg border border-[#FEE2E2] text-xs font-semibold text-[#A32D2D] hover:bg-[#FEE2E2]"
                  >
            ...
            <div className="flex-1 flex items-center justify-center text-[#993556] text-sm">
  ...
  return (
    ...
    >
      <div className="bg-white w-full sm:max-w-2xl rounded-t-2xl sm:rounded-2xl flex flex-col max-h-[96vh] sm:max-h-[92vh] border-t sm:border border-[#F4C0D1] shadow-2xl">
        <div
          className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 rounded-t-2xl flex-shrink-0"
          style={{ background: accentColor }}
        >
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <span className="text-white text-lg sm:text-xl flex-shrink-0">
              {icon}
            </span>
            <div className="min-w-0">
              ...
              <div key={i} className="flex items-center gap-1 flex-shrink-0">
                ...
                {i < totalSteps - 1 && (
                  <div
                    ...
                    style={{
                      background: i < currentStep ? accentColor : "#F4C0D1",
                    }}
                  />
                )}
              </div>
        ...
        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t border-[#F4C0D1] flex flex-col-reverse sm:flex-row justify-between gap-2 bg-[#F9F8F2] flex-shrink-0">
          <button
            onClick={onClose}
            className="px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl border border-[#F4C0D1] text-[#730042] text-xs sm:text-sm font-semibold hover:bg-[#FBEAF0]"
          >
            Cancel
          </button>
          <div className="flex gap-2">
            {!isFirst && (
              <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-1 sm:gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-[#F4C0D1] text-[#730042] text-xs sm:text-sm font-semibold hover:bg-[#FBEAF0]"
              >
            ...
            {!isLast ? (
              <button
                onClick={() => setCurrentStep((s) => s + 1)}
                className="flex-1 sm:flex-none flex items-center justify-center gap-1 sm:gap-1.5 px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl text-white text-xs sm:text-sm font-semibold hover:opacity-90"
                style={{ background: accentColor }}
              >
              ...
              <button
                onClick={onSubmit}
                className="flex-1 sm:flex-none px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl text-white text-xs sm:text-sm font-semibold hover:opacity-90"
                style={{ background: accentColor }}
              >
                Submit
              </button>
    ...
    >
      <div className="bg-white w-full sm:max-w-2xl rounded-t-2xl sm:rounded-2xl flex flex-col max-h-[96vh] sm:max-h-[92vh] border-t sm:border border-[#F4C0D1] shadow-2xl">
        ...
        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t border-[#F4C0D1] flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3 bg-[#F9F8F2] flex-shrink-0">
          ...
          <button
            onClick={onSubmit}
            className="px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl text-white text-xs sm:text-sm font-semibold hover:opacity-90"
            style={{ background: accentColor }}
          >
            Submit
          </button>
    ...
    >
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm p-5 sm:p-6 flex flex-col gap-4 border-t sm:border border-[#F4C0D1] shadow-2xl max-h-[92vh] overflow-y-auto">
        <div className="text-center">
          ...
          <h3 className="text-base sm:text-lg font-bold text-[#730042]">
            {title}
          </h3>
        ...
        <div className="flex flex-col-reverse sm:flex-row gap-3 justify-center">
          <button
            onClick={onCancel}
            className="px-4 sm:px-5 py-2 rounded-xl border border-[#F4C0D1] text-xs sm:text-sm font-semibold text-[#730042] hover:bg-[#FBEAF0]"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            className="px-4 sm:px-5 py-2 rounded-xl text-white text-xs sm:text-sm font-semibold hover:opacity-90"
            style={{ background: confirmColor || "#A32D2D" }}
          >
            {confirmLabel}
          </button>
        </div>
...
function ActionMenu({
  ...
  const isManager =
    effectiveType === "manager" || user.role === "senior_manager";
  ...
  return (
    <div className="relative" onClick={(e) => e.stopPropagation()}>
      <button
        ...
        className="w-7 h-7 lg:w-8 lg:h-8 rounded-lg flex items-center justify-center text-[#993556] border border-[#F4C0D1] hover:bg-[#FBEAF0] flex-shrink-0"
        style={{ background: "#F9F8F2" }}
      >
        <FaEllipsisV size={10} />
      </button>
      {open &&
        createPortal(
          ...
          >
            <button
              onClick={() => {
                onView(user._id, user.role);
                setOpen(false);
              }}
              className="w-full flex items-center gap-2 px-3 py-2 text-xs text-[#730042] hover:bg-[#FBEAF0]"
            >
              ...
              <>
                {isEmployee && (
                  <>
                    <button
                      onClick={() => {
                        onPromoteToManager(user);
                        setOpen(false);
                      }}
                      className="w-full flex items-center gap-2 px-3 py-2 text-xs text-[#3C3489] hover:bg-[#EEEDFE]"
                    >
                      <FaArrowUp size={10} /> Promote to Manager
                    </button>
                    <button
                ...
                {isManager && !isAdmin && (
                  <>
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-3 py-2 text-xs text-[#7A3500] hover:bg-[#FEF3E8]"
                    >
                ...
                {isAdmin && (
                  <>
                    ...
                    <button
                      onClick={() => {
                        onDemoteToEmployee2(user);
                        setOpen(false);
                      }}
                      className="w-full flex items-center gap-2 px-3 py-2 text-xs text-[#A32D2D] hover:bg-[#FCEBEB]"
                    >
                      <FaArrowDown size={10} /> Demote to Employee
                    </button>
                  </>
...
function roleBadge(u) {
  ...
  if (u.role === "senior_manager")
    return <Badge label="Sr. Manager" type="smgr" />;
  if (u.role === "admin" || u.role === "senior_admin")
    return <Badge label="Admin" type="admin" />;
    ...
    <tr key={i} className="border-b border-[#FBEAF0]">
      {Array.from({ length: 8 }).map((_, j) => (
        <td key={j} className="px-4 py-3">
          <div
            className="h-4 bg-[#FBEAF0] rounded animate-pulse"
...
function MobileSkeletons() {
  return Array.from({ length: 4 }).map((_, i) => (
    <div
      key={i}
      className="bg-white border border-[#F4C0D1] rounded-xl p-4 animate-pulse"
    >
    ...
    <tr>
      <td colSpan={8}>
        <div className="flex flex-col items-center justify-center py-12 sm:py-16 gap-3 text-center px-4">
          ...
          <p className="text-[#730042] font-medium text-sm sm:text-base">
            No employees found
          </p>
          <p className="text-[#993556] text-xs sm:text-sm">
            Add your first employee to get started
          </p>
          <button
            onClick={onAdd}
            className="mt-2 px-4 py-2 rounded-xl text-white text-sm font-semibold hover:opacity-90 transition"
            style={{ background: "#730042" }}
          >
            + Add Employee
          </button>
...
function Popup({ type = "success", message, onClose }) {
  ...
  return (
    <div
      className="fixed top-4 right-4 sm:top-5 sm:right-5 z-[100] max-w-[calc(100vw-2rem)]"
    ...
    >
      ...
      <div
        className="min-w-[240px] sm:min-w-[280px] max-w-sm px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl bg-white border-2 flex items-start justify-between gap-3 shadow-lg"
        style={{ borderColor: "#730042" }}
      >
        <span
...
function FilterChip({ label, onRemove }) {
  return (
    <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#FBEAF0] text-[#730042] text-xs font-medium border border-[#F4C0D1]">
      {label}
      <button onClick={onRemove} className="hover:text-[#CD166E]">
        <FaTimes size={9} />
      </button>
    </span>
  );
...
function MobileCard({
  ...
  const effectiveType =
    ...
    (u.role === "manager" || u.role === "senior_manager"
      ...
      : "employee");
  const roleType =
    effectiveType === "manager"
      ? u.role === "senior_manager"
        ? "smgr"
        : "manager"
      : u.role === "admin" || u.role === "senior_admin"
        ? "admin"
        : "role";
  const roleLabel =
    effectiveType === "manager"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2 flex-wrap">
          <div className="min-w-0">
            <p
              className={`font-semibold text-sm truncate ${isInactive ? "text-[#6B7280]" : "text-[#730042]"}`}
            >
              {u.f_name} {u.l_name}
            </p>
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          {u.department && (
            <Badge
              label={DEPT_FULL_FORMS[u.department] || u.department}
              type="dept"
            />
          )}
          {u.office_location && (
        ...
        <div className="flex items-center justify-between mt-3 gap-2">
          {u.Under_manager ? (
            <p className="text-[11px] text-[#993556] truncate">
              Under:{" "}
              <span className="font-medium text-[#730042]">
            ...
            <p className="text-[11px] text-[#993556] truncate">
              Reports to:{" "}
              <span className="font-medium text-[#730042]">
                {u.reporting_manager.f_name} {u.reporting_manager.l_name}
              </span>
            </p>
          ) : (
            <span />
          ...
          <div className="flex-shrink-0" onClick={(e) => e.stopPropagation()}>
            <ActionMenu
              ...
              onDemoteToEmployee={onDemoteToEmployee}
...
function FieldWorkOnboardingFields({ form, onChange }) {
  const teams = useFieldTeams(Boolean(form.isFieldEmployee));
  return (
    <div className="rounded-lg border border-dashed border-[#CD166E]/40 bg-[#FDF3F8] p-3">
      <label className="flex items-center gap-2 text-sm font-semibold text-[#730042]">
        <input
          type="checkbox"
          checked={Boolean(form.isFieldEmployee)}
          onChange={(e) =>
            onChange({
              target: { name: "isFieldEmployee", value: e.target.checked },
            })
          }
        />
        Is this a Field Employee?
      </label>
      {form.isFieldEmployee && (
        <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
          <select
            name="fieldTeamId"
            value={form.fieldTeamId}
            onChange={onChange}
            className={inputCls}
          >
          ...
          <input
            name="fieldTerritory"
            placeholder="Territory (optional, used if no team)"
            value={form.fieldTerritory}
            onChange={onChange}
            className={inputCls}
          />
          {teams.isError && (
            <p className="col-span-1 text-xs text-[#993556] sm:col-span-2">
              Field Work team list isn't available right now — the employee will
              still be marked as a field employee without a team; assign one
              later from Field Operations.
            </p>
          )}
        </div>
      )}
...
function EmpStepFields({
  ...
  if (step === 0)
    return (
      <>
        ...
        <Field label="Last Name" error={errors.l_name}>
          <input
            name="l_name"
        ...
        <Field
          ...
          error={
            ...
            form.e_contact === form.personal_contact
  ...
  if (step === 1)
    return (
      <>
        ...
        <div className="col-span-1 sm:col-span-2">
          <FieldWorkOnboardingFields form={form} onChange={onChange} />
        </div>
      </>
    );
  if (step === 2)
    return <AddressFields form={form} onChange={onChange} errors={errors} />;
  if (step === 3)
    return (
      <>
  ...
  if (step === 5)
    return (
      <>
        <Field label="Bank Name" error={errors.bank_group}>
          <input
            name="bank_name"
        ...
        <div className="col-span-1 sm:col-span-2">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-[#993556] mb-3">
            Documents (optional)
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <FileUploadField
  ...
  if (step === 6)
    return (
      <div className="col-span-1 sm:col-span-2">
        <p className="text-xs text-[#993556] mb-1">
          Set initial permissions for this employee.
        </p>
        <p className="text-[11px] text-[#993556]/70 mb-3">
          Greyed toggles are restricted by role and cannot be enabled.
        </p>
        <PermissionsPanel
          perms={perms}
...
function MgrStepFields({
  ...
  if (step === 6)
    return (
      <div className="col-span-1 sm:col-span-2">
        <p className="text-xs text-[#993556] mb-1">
          Set initial permissions for this manager.
        </p>
        <p className="text-[11px] text-[#993556]/70 mb-3">
          Greyed toggles are restricted by role and cannot be enabled.
        </p>
...
function validateContactInfo(form) {
  ...
  if (form.l_name && !NAME_REGEX.test(form.l_name))
    err.l_name = "Enter a valid name";

  if (!form.work_email) err.work_email = "Required";
  else if (!EMAIL_REGEX.test(form.work_email))
    err.work_email = "Invalid email address";
  ...
  else if (!PHONE_REGEX.test(form.personal_contact))
    err.personal_contact = "Must be a valid 10-digit Indian mobile number";
  if (!form.e_contact) err.e_contact = "Required";
  else if (!PHONE_REGEX.test(form.e_contact))
    err.e_contact = "Must be a valid 10-digit Indian mobile number";
...
function validateAddressInfo(form) {
  ...
  if (!form.pincode) err.pincode = "Required";
    ...
    err.pincode = "Must be a valid 6-digit pincode";
  if (form.permanent_pincode && !PINCODE_REGEX.test(form.permanent_pincode))
    err.permanent_pincode = "Must be a valid 6-digit pincode";
  return err;
}
...
function validateIdentityInfo(form) {
  ...
  if (form.aadhaar_number && !AADHAAR_REGEX.test(form.aadhaar_number))
    err.aadhaar_number = "Must be exactly 12 digits";
  if (form.pan_number && !PAN_REGEX.test(form.pan_number))
    err.pan_number = "Invalid PAN format (e.g. ABCDE1234F)";
  return err;
}
...
function validateExperienceInfo(form) {
  ...
  if (!form.is_fresher && form.total_experience) {
    ...
    if (isNaN(exp) || exp < 0 || exp > 50)
      err.total_experience = "Enter a valid experience (0-50 years)";
  }
  return err;
}
...
function validateBankInfo(form) {
  ...
  if (hasBankPartial && !hasBankFull)
    err.bank_group = "Fill all bank details together or leave all blank";
  if (form.account_holder_name && !NAME_REGEX.test(form.account_holder_name))
    err.account_holder_name = "Only letters allowed, no numbers";
  if (form.account_number && !ACCOUNT_REGEX.test(form.account_number))
    err.account_number = "Account number must be 9-18 digits";
  if (form.ifsc_code && !IFSC_REGEX.test(form.ifsc_code))
    err.ifsc_code = "Invalid IFSC code format (e.g. SBIN0001234)";
  if (form.resume && !URL_REGEX.test(form.resume))
    err.resume = "Must be a valid URL starting with https://";
  if (form.aadhaar_card && !URL_REGEX.test(form.aadhaar_card))
    err.aadhaar_card = "Must be a valid URL starting with https://";
  if (form.pan_card && !URL_REGEX.test(form.pan_card))
    err.pan_card = "Must be a valid URL starting with https://";
  if (form.experience_letter && !URL_REGEX.test(form.experience_letter))
    err.experience_letter = "Must be a valid URL starting with https://";
  return err;
}
...
const FIELD_LABELS = {
  ...
  e_contact: "Emergency Contact",
  department: "Department",
  designation: "Designation",
  office_location_country: "Office Country",
  office_location_state: "Office State",
  ...
  bank_group: "Bank Details",
  account_number: "Account Number",
  ifsc_code: "IFSC Code",
  account_holder_name: "Account Holder Name",
  bank_name: "Bank Name",
  resume: "Resume URL",
  aadhaar_card: "Aadhaar Card URL",
  pan_card: "PAN Card URL",
  experience_letter: "Experience Letter URL",
};
...
const FIELD_STEP_MAP = {
  ...
  department: 1,
  designation: 1,
  office_location_country: 1,
  office_location_state: 1,
  office_location: 1,
  address: 2,
  country: 2,
  state: 2,
...
function getErrorSummary(err) {
  ...
  return {
    message: `Fix these fields: ${labels.join(", ")} — go to "${stepLabel}" step`,
    step,
  };
}
...
export default function EmployeeTable() {
  ...
  const [promoteToMgrForm, setPromoteToMgrForm] = useState({
    reporting_manager: "",
    designation: "",
    role: "manager",
  });
  const [promoteToAdminTarget, setPromoteToAdminTarget] = useState(null);
  const [promoteToAdminForm, setPromoteToAdminForm] = useState({
    reporting_manager: "",
    designation: "",
    role: "admin",
  });
  const [demoteMgrToEmpTarget, setDemoteMgrToEmpTarget] = useState(null);
  const [demoteMgrToEmpForm, setDemoteMgrToEmpForm] = useState({
    Under_manager: "",
    designation: "",
  });
  const [demoteAdminToMgrTarget, setDemoteAdminToMgrTarget] = useState(null);
  const [demoteAdminToMgrForm, setDemoteAdminToMgrForm] = useState({
    reporting_manager: "",
    designation: "",
    role: "manager",
  });
  const [demoteAdminToEmpTarget, setDemoteAdminToEmpTarget] = useState(null);
  const [demoteAdminToEmpForm, setDemoteAdminToEmpForm] = useState({
    Under_manager: "",
    designation: "",
  });
  ...
  const validateEdit = () => {
    ...
    else if (!EMAIL_REGEX.test(editForm.work_email))
      err.work_email = "Invalid email address";
    if (!editForm.designation?.trim()) err.designation = "Required";
    if (!editForm.department) err.department = "Required";
    if (
      editForm.personal_contact &&
      !PHONE_REGEX.test(editForm.personal_contact)
    )
      err.personal_contact = "Must be a valid 10-digit Indian mobile number";
    if (editForm.e_contact && !PHONE_REGEX.test(editForm.e_contact))
      err.e_contact = "Must be a valid 10-digit Indian mobile number";
    else if (
      editForm.personal_contact &&
      editForm.e_contact &&
      editForm.e_contact === editForm.personal_contact
    ) {
      err.e_contact =
        "Emergency contact must be different from personal contact";
    }
  ...
  const handlePromoteToManager = () => {
    promoteToMgrApi(
      { id: promoteToMgrTarget._id, data: promoteToMgrForm },
      {
        onSuccess: (res) => {
          showPopup("success", res?.message || "Promoted to Manager");
          setPromoteToMgrTarget(null);
          refetchList();
        },
        onError: (err) =>
          showPopup(
  ...
  const handlePromoteToAdmin = () => {
    const isManager =
      promoteToAdminTarget.role === "manager" ||
      promoteToAdminTarget.role === "senior_manager";
    const mutate = isManager
      ? promoteToAdminFromMgrApi
      : promoteToAdminFromEmpApi;
    mutate(
      { id: promoteToAdminTarget._id, data: promoteToAdminForm },
      {
        onSuccess: (res) => {
          showPopup("success", res?.message || "Promoted to Admin");
          setPromoteToAdminTarget(null);
          refetchList();
        },
        onError: (err) =>
          showPopup(
  ...
  const handleDemoteMgrToEmp = () => {
    demoteMgrToEmpApi(
      { id: demoteMgrToEmpTarget._id, data: demoteMgrToEmpForm },
      {
        onSuccess: (res) => {
          showPopup("success", res?.message || "Demoted to Employee");
          setDemoteMgrToEmpTarget(null);
          refetchList();
        },
        onError: (err) =>
          showPopup(
            "error",
            err?.response?.data?.message || err?.message || "Demotion failed",
          ),
      },
    );
  };
  ...
  const handleDemoteAdminToMgr = () => {
    demoteAdminToMgrApi(
      { id: demoteAdminToMgrTarget._id, data: demoteAdminToMgrForm },
      {
        onSuccess: (res) => {
          showPopup("success", res?.message || "Demoted to Manager");
          setDemoteAdminToMgrTarget(null);
          refetchList();
        },
        onError: (err) =>
          showPopup(
  ...
  const handleDemoteAdminToEmp = () => {
    demoteAdminToEmpApi(
      { id: demoteAdminToEmpTarget._id, data: demoteAdminToEmpForm },
      {
        onSuccess: (res) => {
          showPopup("success", res?.message || "Demoted to Employee");
          setDemoteAdminToEmpTarget(null);
          refetchList();
        },
        onError: (err) =>
          showPopup(
  ...
  const makeChangeHandler = (setter) => (e) => {
    ...
    if (
      ["f_name", "l_name", "account_holder_name"].includes(name) &&
      value &&
      !/^[A-Za-z\s.'-]*$/.test(value)
    ) {
      return;
    ...
    setter((prev) => {
      ...
      if (name === "office_location_country") {
        next.office_location_state = "";
        next.office_location = "";
      }
      if (name === "office_location_state") {
        next.office_location = "";
      }
    ...
    addEmployeeApi(
      {
        ...
        city: empForm.city,
        state: empForm.state,
        pincode: empForm.pincode,
        country: empForm.country,
        permanent_address: empForm.permanent_address || undefined,
        permanent_city: empForm.permanent_city || undefined,
        permanent_state: empForm.permanent_state || undefined,
        permanent_pincode: empForm.permanent_pincode || undefined,
        ...
        total_experience: empForm.is_fresher
          ? undefined
          : empForm.total_experience || undefined,
        previous_company: empForm.is_fresher
          ? undefined
          : empForm.previous_company || undefined,
        previous_designation: empForm.is_fresher
          ? undefined
          : empForm.previous_designation || undefined,
        ...
        isFieldEmployee: empForm.isFieldEmployee || undefined,
        fieldTeamId: empForm.isFieldEmployee
          ? empForm.fieldTeamId || undefined
          : undefined,
        fieldTerritory: empForm.isFieldEmployee
          ? empForm.fieldTerritory || undefined
          : undefined,
      },
      {
        onSuccess: (res) => {
          showPopup(
            "success",
            res?.fieldWorkNote
              ? `${res?.message || "Employee added successfully"} ${res.fieldWorkNote}`
          ...
          setEmpErrors({});
          setEmpStep(0);
          setEmpPerms({ ...EMP_DEFAULT_PERMISSIONS });
          refetchList();
        },
        onError: (err) =>
          showPopup(
    ...
    addManagerApi(
      {
        ...
        address: mgrForm.address,
        city: mgrForm.city,
        state: mgrForm.state,
        pincode: mgrForm.pincode,
        country: mgrForm.country,
        permanent_address: mgrForm.permanent_address || undefined,
        permanent_city: mgrForm.permanent_city || undefined,
        permanent_state: mgrForm.permanent_state || undefined,
        permanent_pincode: mgrForm.permanent_pincode || undefined,
        ...
        total_experience: mgrForm.is_fresher
          ? undefined
          : mgrForm.total_experience || undefined,
        previous_company: mgrForm.is_fresher
          ? undefined
          : mgrForm.previous_company || undefined,
        previous_designation: mgrForm.is_fresher
          ? undefined
          : mgrForm.previous_designation || undefined,
      ...
      },
      {
        onSuccess: (res) => {
          ...
          setMgrErrors({});
          setMgrStep(0);
          setMgrPerms({ ...MGR_DEFAULT_PERMISSIONS });
          refetchList();
        },
        onError: (err) =>
          showPopup(
  ...
  const filtered = allUsers.filter((u) => {
    ...
    return (
      ...
      (filters.working_status
        ? u.working_status === filters.working_status
        : true) &&
      matchType
    );
  });
  ...
  const openPromoteToManager = (user) => {
    setPromoteToMgrTarget(user);
    setPromoteToMgrForm({
      reporting_manager: "",
      designation: user.designation || "",
      role: "manager",
  ...
  const openPromoteToAdmin = (user) => {
    setPromoteToAdminTarget(user);
    setPromoteToAdminForm({
      reporting_manager: "",
      designation: user.designation || "",
      role: "admin",
    });
  };
  const openDemoteMgrToEmp = (user) => {
    setDemoteMgrToEmpTarget(user);
    setDemoteMgrToEmpForm({
      Under_manager: "",
      designation: user.designation || "",
    });
  };
  const openDemoteAdminToMgr = (user) => {
    setDemoteAdminToMgrTarget(user);
    setDemoteAdminToMgrForm({
      reporting_manager: "",
      designation: user.designation || "",
      role: "manager",
    });
  };
  const openDemoteAdminToEmp = (user) => {
    setDemoteAdminToEmpTarget(user);
    setDemoteAdminToEmpForm({
      Under_manager: "",
      designation: user.designation || "",
    });
  };
    ...
    >
      <div className="max-w-7xl w-full h-full flex flex-col min-h-0">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 flex-shrink-0">
          <div className="min-w-0">
            <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-[#730042] tracking-tight">
              Employee Directory
            </h1>
            <p className="text-[11px] sm:text-xs md:text-sm text-[#993556] mt-0.5 break-words">
              {activeUsers.length} active · {filtered.length} shown ·{" "}
              {employeeData?.employees ?? 0} employees ·{" "}
              {employeeData?.managers ?? 0} managers
              {inactiveCount > 0 && (
                <>
                  {" "}
                  ·{" "}
              ...
              {allowedUsers !== null && (
                <>
                  ...
                  >
                    {remainingSlots} User{remainingSlots !== 1 ? "s" : ""}{" "}
                    remaining of {allowedUsers}
                  </span>
                </>
              )}
            </p>
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2">
            ...
            <button
              onClick={() => {
                const managerFullMap = new Map(
                  (managersWithAdmin?.managers ?? []).map((m) => [m._id, m]),
                );
                const employeeFullMap = new Map(
                  (employeesFull?.employees ?? []).map((e) => [e._id, e]),
                );
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.background = "#085041";
            ...
            {isLimitReached ? (
              <div className="col-span-2 sm:col-span-auto flex items-center justify-center gap-2 px-3 sm:px-4 py-2 rounded-xl border-2 border-[#FCA5A5] bg-[#FFF5F5] text-xs sm:text-sm font-semibold text-[#991B1B]">
                ...
                <span className="hidden sm:inline">
                  Limit reached — contact your organization
                </span>
              ...
              <>
                ...
                <button
                  onClick={() => setOpenBulkOnboard(true)}
                  className="flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-xl border-2 text-xs sm:text-sm font-semibold transition-all w-full sm:w-auto"
                  style={{ borderColor: "#085041", color: "#085041" }}
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = "#085041";
        ...
        {showInactive && inactiveCount > 0 && (
          <div className="mb-3 flex items-center gap-2 px-3 py-2 rounded-xl border border-[#E5E7EB] bg-[#F9FAFB] text-xs text-[#6B7280] flex-shrink-0">
            ...
            <span>
              Showing <strong>{inactiveCount}</strong> inactive user
              {inactiveCount !== 1 ? "s" : ""} (resigned / fired / terminated).
              Their profiles are read-only.
            </span>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-[#F4C0D1] overflow-hidden flex flex-col flex-1 min-h-0">
          <div
            className="p-3 sm:p-4 border-b border-[#F4C0D1] flex-shrink-0"
            style={{ background: "#F9F8F2" }}
          >
            <div className="flex gap-2 mb-2">
              <div className="relative flex-1">
                <FaSearch
                  className="absolute left-3 top-1/2 -translate-y-1/2 text-[#993556]"
                  size={12}
                />
                <input
              ...
              <div className="mt-2 pt-2 border-t border-[#F4C0D1]">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFilters({ ...filters, working_status: e.target.value })
                    }
                  >
                ...
                {activeFilterCount > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2 items-center">
                    ...
                    {filters.department && (
                      <FilterChip
                        label={`Dept: ${DEPT_FULL_FORMS[filters.department] || filters.department}`}
                        onRemove={() =>
                    ...
                    {filters.working_status && (
                      <FilterChip
                        label={`Employment: ${filters.working_status}`}
                        onRemove={() =>
                          setFilters({ ...filters, working_status: "" })
                        }
                      />
                    )}
                    <button
                      onClick={clearFilters}
                      className="text-xs text-[#A32D2D] font-semibold hover:underline ml-1"
                    >
                      Clear All
                    </button>
                  </div>
          ...
          <div
            className="lg:hidden p-3 space-y-2.5 overflow-y-auto flex-1 min-h-0"
            style={{ background: "#F9F8F2" }}
          >
            {listLoading ? (
              <MobileSkeletons />
              ...
              <div className="flex flex-col items-center justify-center py-12 gap-3 text-center">
                ...
                <p className="text-[#730042] font-medium text-sm">
                  No employees found
                </p>
                {!isLimitReached && (
                  <button
                    onClick={() => setOpen(true)}
                    className="mt-1 px-4 py-2 rounded-xl text-white text-sm font-semibold hover:opacity-90"
                    style={{ background: "#730042" }}
                  >
                    + Add Employee
                  </button>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#FBEAF0]">
                {listLoading ? (
                  ...
                  <EmptyState onAdd={() => !isLimitReached && setOpen(true)} />
                ) : (
                  filtered.map((u) => {
                    const isInactive =
                      u.working_status && u.working_status !== "working";
                    return (
                      <tr
                        key={u._id}
                        className={`transition-colors group cursor-pointer ${isInactive ? "opacity-70" : ""}`}
                        onMouseEnter={(e) =>
                          (e.currentTarget.style.background = isInactive
                            ? "#F9FAFB"
                            : "#FEF4F9")
                        }
                        onMouseLeave={(e) =>
                      ...
                      >
                        <td className="px-3 lg:px-4 py-3">
                          <div className="flex items-center gap-2 lg:gap-3">
                            ...
                            <div className="min-w-0">
                              <p
                                className={`font-semibold text-xs lg:text-sm truncate max-w-[120px] lg:max-w-[160px] ${isInactive ? "text-[#6B7280]" : "text-[#730042]"}`}
                              >
                                {u.f_name} {u.l_name}
                              </p>
                              <p className="text-[11px] text-[#993556] truncate max-w-[120px] lg:max-w-[160px]">
                                {u.work_email}
                              </p>
                              {u.empid && (
                                <p className="text-[10px] text-[#993556]/60 font-mono">
                                  {u.empid}
                                </p>
                              )}
                            </div>
                        ...
                        <td className="px-3 lg:px-4 py-3 text-[#730042] text-xs lg:text-sm max-w-[100px] lg:max-w-none truncate">
                          {u.designation || "—"}
                        </td>
                          ...
                          {u.Under_manager ? (
                            <div className="text-xs">
                              <p className="font-medium text-[#730042] truncate max-w-[80px] lg:max-w-none">
                                {u.Under_manager.f_name}{" "}
                                {u.Under_manager.l_name}
                              </p>
                              <p className="text-[#993556] text-[10px] hidden lg:block">
                                {u.Under_manager.empid}
                              </p>
                            </div>
                          ) : u.reporting_manager ? (
                            <div className="text-xs">
                              <p className="font-medium text-[#730042] truncate max-w-[80px] lg:max-w-none">
                                {u.reporting_manager.f_name}{" "}
                                {u.reporting_manager.l_name}
                              </p>
                              <p className="text-[#993556] text-[10px] hidden lg:block">
                                {u.reporting_manager.empid}
                              </p>
                            </div>
                          ) : (
          ...
          {!listLoading && filtered.length > 0 && (
            <div
              className="px-3 sm:px-4 py-2.5 sm:py-3 border-t border-[#F4C0D1] text-[11px] sm:text-xs text-[#993556] flex items-center justify-between gap-2 flex-shrink-0"
              style={{ background: "#F9F8F2" }}
            >
              <span>
                Showing {filtered.length} of {allUsers.length}
              </span>
              {activeFilterCount > 0 && (
                <button
                  onClick={clearFilters}
                  className="text-[#A32D2D] font-medium hover:underline flex-shrink-0"
                >
                  Clear filters
                </button>
              )}
      ...
      <BulkOnboardingModal
        open={openBulkOnboard}
        onClose={() => setOpenBulkOnboard(false)}
      />
      ...
      {openEdit && editTarget && (
        <Modal
          ...
          icon={
            ...
            editTarget.role === "senior_manager" ? (
              <FaUserTie />
            ) : (
              <FaUserPlus />
            )
        ...
        >
          ...
          <Field label="Phone" error={editErrors.personal_contact}>
            <input
              name="personal_contact"
              value={editForm.personal_contact}
              onChange={handleEditChange}
              className={inputCls}
            />
          </Field>
      ...
      {promoteToMgrTarget && (
        <ConfirmModal
          ...
          confirmColor="#3C3489"
          onConfirm={handlePromoteToManager}
          onCancel={() => setPromoteToMgrTarget(null)}
        >
          <div className="flex flex-col gap-2 -mt-1">
            <Field label="New Designation">
              <input
                ...
                value={promoteToMgrForm.designation}
                onChange={(e) =>
                  setPromoteToMgrForm({
                    ...promoteToMgrForm,
            ...
            <ReportingManagerSelect
              ...
              label="Reporting Manager"
              name="reporting_manager"
              excludeId={promoteToMgrTarget?._id}
            />
          </div>
        </ConfirmModal>
      )}
      ...
      {promoteToAdminTarget && (
        <ConfirmModal
          ...
          onCancel={() => setPromoteToAdminTarget(null)}
        >
          <div className="flex flex-col gap-2 -mt-1">
            <Field label="New Designation">
              <input
                ...
                value={promoteToAdminForm.designation}
                onChange={(e) =>
                  setPromoteToAdminForm({
                    ...promoteToAdminForm,
                    designation: e.target.value,
            ...
            <div className="text-xs text-[#7A3500] bg-[#FEF3C7] border border-[#FDE68A] rounded-lg px-3 py-2">
              As an Admin, {promoteToAdminTarget.f_name} will report directly to
              the Super Admin.
            </div>
          </div>
        </ConfirmModal>
      ...
      {demoteMgrToEmpTarget && (
        <ConfirmModal
          ...
          confirmLabel="Demote"
          confirmColor="#7A3500"
          onConfirm={handleDemoteMgrToEmp}
          onCancel={() => setDemoteMgrToEmpTarget(null)}
        >
          <div className="flex flex-col gap-2 -mt-1">
            <Field label="New Designation">
              <input
                ...
                value={demoteMgrToEmpForm.designation}
                onChange={(e) =>
                  setDemoteMgrToEmpForm({
                    ...demoteMgrToEmpForm,
                    designation: e.target.value,
            ...
            <UnderManagerSelect
              value={demoteMgrToEmpForm.Under_manager}
              onChange={(e) =>
                setDemoteMgrToEmpForm({
                  ...demoteMgrToEmpForm,
      ...
      {demoteAdminToMgrTarget && (
        <ConfirmModal
          title="Demote Admin to Manager?"
          icon="⬇️"
          message={`Demote ${demoteAdminToMgrTarget.f_name} ${demoteAdminToMgrTarget.l_name} from Admin to Manager.`}
          confirmLabel="Demote"
          confirmColor="#7A3500"
          onConfirm={handleDemoteAdminToMgr}
          onCancel={() => setDemoteAdminToMgrTarget(null)}
        >
          <div className="flex flex-col gap-2 -mt-1">
            <Field label="New Designation">
              <input
                className={inputCls}
                placeholder="e.g. Team Lead"
                value={demoteAdminToMgrForm.designation}
                onChange={(e) =>
                  setDemoteAdminToMgrForm({
                    ...demoteAdminToMgrForm,
                    designation: e.target.value,
                  })
                }
              />
            ...
            <Field label="Manager Role">
              <select
                className={inputCls}
                value={demoteAdminToMgrForm.role}
                onChange={(e) =>
                  setDemoteAdminToMgrForm({
                    ...demoteAdminToMgrForm,
                    role: e.target.value,
          ...
          <div className="flex flex-col gap-2 -mt-1">
            <Field label="New Designation">
              <input
                ...
                value={demoteAdminToEmpForm.designation}
                onChange={(e) =>
                  setDemoteAdminToEmpForm({
                    ...demoteAdminToEmpForm,
                    designation: e.target.value,
                  })
                }
              />
            ...
            <UnderManagerSelect
              value={demoteAdminToEmpForm.Under_manager}
              onChange={(e) =>
                setDemoteAdminToEmpForm({
                  ...demoteAdminToEmpForm,
                  Under_manager: e.target.value,
