...
function Field({ label, error, children, required, span2 }) {
  return (
    <div className={`flex flex-col gap-1 min-w-0 ${span2 ? "col-span-1 sm:col-span-2" : ""}`}>
      ...
      {children}
      {error && <span className="text-[11px] text-[#A32D2D] flex items-start gap-1 break-words">⚠ {error}</span>}
    </div>
  );
}
...
function StepModal({ title, icon, onClose, onSubmit, steps, currentStep, setCurrentStep, children, accentColor = "#CD166E" }) {
  ...
  return (
    ...
    >
      <div className="bg-white w-full sm:max-w-2xl rounded-t-2xl sm:rounded-2xl flex flex-col max-h-[96vh] sm:max-h-[92vh] border-t sm:border border-[#F4C0D1] shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between gap-2 px-4 sm:px-6 py-3 sm:py-4 rounded-t-2xl flex-shrink-0" style={{ background: accentColor }}>
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] sm:text-xs hidden xs:block truncate" style={{ color: "rgba(255,255,255,0.65)" }}>
        ...
        <div className="px-3 sm:px-6 pt-3 pb-2 bg-white border-b border-[#F4C0D1] flex-shrink-0 min-w-0">
          <div className="flex items-center gap-1 overflow-x-auto pb-1 scrollbar-hide">
            {steps.map((s, i) => (
              <div key={i} className="flex items-center gap-1 flex-shrink-0">
                <button
                  onClick={() => setCurrentStep(i)}
                  className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1.5 rounded-lg text-[11px] sm:text-xs font-semibold transition-all whitespace-nowrap"
        ...
        <div className="overflow-y-auto overflow-x-hidden p-3 sm:p-6 flex-1 bg-[#F9F8F2] min-w-0">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 lg:gap-6">{children}</div>
        </div>

        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t border-[#F4C0D1] flex flex-wrap justify-between gap-2 bg-[#F9F8F2] flex-shrink-0">
      ...
      <div className="bg-white w-full sm:max-w-2xl rounded-t-2xl sm:rounded-2xl flex flex-col max-h-[96vh] sm:max-h-[92vh] border-t sm:border border-[#F4C0D1] shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between gap-2 px-4 sm:px-6 py-3 sm:py-4 rounded-t-2xl flex-shrink-0" style={{ background: accentColor }}>
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-base sm:text-lg font-bold text-white truncate">{title}</h2>
              <p className="text-[11px] sm:text-xs truncate" style={{ color: "rgba(255,255,255,0.6)" }}>Fill in all required fields</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-white flex-shrink-0" style={{ background: "rgba(255,255,255,0.18)" }}>
            <FaTimes size={13} />
          </button>
        </div>
        <div className="overflow-y-auto overflow-x-hidden p-3 sm:p-6 flex-1 bg-[#F9F8F2] min-w-0">
        ...
        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t border-[#F4C0D1] flex flex-wrap justify-end gap-2 sm:gap-3 bg-[#F9F8F2] flex-shrink-0">
...
function MobileCard({ u, onView, onEdit, onDelete, onPermissions }) {
  ...
  return (
    <div
      className="bg-white border border-[#F4C0D1] rounded-xl p-4 flex gap-3 cursor-pointer active:scale-[0.99] transition-transform min-w-0"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          ...
          {u.office_location && (
            <span className="px-2 py-0.5 rounded-full text-xs bg-[#F9F8F2] text-[#993556] border border-[#F4C0D1] truncate max-w-[140px]">
              📍 {u.office_location}
            </span>
          )}
          {u.designation && (
        ...
        <div className="flex items-center justify-between gap-2 mt-3 flex-wrap">
          {u.Under_manager ? (
            <p className="text-[11px] text-[#993556] min-w-0 truncate">
...
function Popup({ type = "success", message, onClose }) {
  ...
  return (
    <div className="fixed top-4 right-4 left-4 sm:left-auto sm:top-5 sm:right-5 z-[100] max-w-full sm:max-w-[calc(100vw-2rem)]" style={{ animation: "slideInPopup 0.3s ease forwards" }}>
      <style>{`@keyframes slideInPopup { from { opacity:0; transform:translateX(60px); } to { opacity:1; transform:translateX(0); } }`}</style>
      <div className="w-full sm:min-w-[280px] sm:max-w-sm px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl text-white flex items-start justify-between gap-3" style={styles[type]}>
        <span className="text-xs sm:text-sm font-medium break-words min-w-0">{message}</span>
...
function FilterChip({ label, onRemove }) {
  return (
    <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#FBEAF0] text-[#730042] text-xs font-medium border border-[#F4C0D1] max-w-full">
      <span className="truncate">{label}</span>
      <button onClick={onRemove} className="hover:text-[#CD166E] transition-colors flex-shrink-0"><FaTimes size={9} /></button>
    </span>
  );
}
...
function DeleteConfirm({ user, onConfirm, onCancel }) {
  return (
    <div className="fixed inset-0 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4" style={{ background: "rgba(115,0,66,0.40)", backdropFilter: "blur(3px)" }}>
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm p-5 sm:p-6 flex flex-col gap-4 border-t sm:border border-[#F4C0D1] shadow-2xl max-h-[96vh] overflow-y-auto">
        <div className="text-center min-w-0">
          <div className="text-3xl sm:text-4xl mb-2">🗑️</div>
          <h3 className="text-base sm:text-lg font-bold text-[#730042]">Delete User?</h3>
          <p className="text-xs sm:text-sm text-[#993556] mt-1 break-words">
...
export default function EmployeeTable() {
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden p-3 sm:p-4 md:p-6 lg:p-8 font-['DM_Sans',system-ui,sans-serif]" style={{ background: "#F9F8F2" }}>
      <style>{`
      ...
      <div className="max-w-7xl mx-auto w-full min-w-0">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold text-[#730042] tracking-tight truncate">Employee Directory</h1>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-xl border-2 text-xs sm:text-sm font-semibold hover:text-white transition-all whitespace-nowrap"
            ...
            <button
              ...
              className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-xl text-white text-xs sm:text-sm font-semibold hover:opacity-90 transition-all whitespace-nowrap"
        ...
        <div className="bg-white rounded-2xl border border-[#F4C0D1] overflow-hidden w-full min-w-0">
                      ...
                      <td className="px-3 lg:px-4 py-3">
                        {u.Under_manager ? (
                          <div className="text-xs min-w-0">
                            ...
                            <p className="text-[#993556] hidden lg:block truncate">{u.Under_manager.uid}</p>
                          </div>
          ...
          {!listLoading && filtered.length > 0 && (
            <div className="px-3 sm:px-4 py-2.5 sm:py-3 border-t border-[#F4C0D1] text-[11px] sm:text-xs text-[#993556] flex flex-wrap items-center justify-between gap-2" style={{ background: "#F9F8F2" }}>
