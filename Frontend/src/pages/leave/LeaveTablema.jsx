...
const LEAVE_STATUS_META = {
  ...
  "pending_r-manager":        { label:"Pending RM",      bg:"#FFFBEB", color:"#92400E", dot:"#F59E0B" },
  ...
  "rejected_r-manager":       { label:"Rejected",        bg:"#FEF2F2", color:"#991B1B", dot:"#EF4444" },
...
const MyBalancePanel = ({manager}) => {
  const {data:rawData,isLoading} = useGetMyLeavesManager();
  if (isLoading) return <Spinner/>;

  const balanceArr = Array.isArray(rawData) ? rawData : [];
...
const ApplyLeavePanel = ({manager,showToast}) => {
  ...
  const availTypes = APPLY_LEAVE_TYPES.filter(t=>{
    if (t.value==="ml") return manager?.gender==="female"&&manager?.marital_status==="married";
    if (t.value==="pl") return manager?.gender==="male"  &&manager?.marital_status==="married";
    return true;
  });
...
const MyWFHPanel = ({showToast}) => {
  ...
  const wfhList = wfhData?.wfhList||wfhData||[];
  const list = Array.isArray(wfhList)?wfhList:[];
...
const TeamWFHPanel = ({showToast}) => {
  ...
  const pendingList   = pendingData?.wfhList||pendingData||[];
  const teamList      = teamData?.wfhList||teamData||[];
  const forwardedList = fwdData?.wfhList||fwdData||[];
  ...
  const innerTabs = [
    {key:"pending",   label:`Pending (${Array.isArray(pendingList)?pendingList.length:0})`},
    {key:"all",       label:`All Team (${Array.isArray(teamList)?teamList.length:0})`},
    {key:"forwarded", label:`Forwarded (${Array.isArray(forwardedList)?forwardedList.length:0})`},
  ];
  ...
  return (
    <div>
      ...
      {activeTab==="pending"&&(
        ...
        !Array.isArray(pendingList)||pendingList.length===0?<EmptyState msg="No pending WFH requests"/>:
        pendingList.map((wfh,idx)=>renderWFHCard(wfh,idx,pendingActions))
      )}
      {activeTab==="all"&&(
        teamLoading?<Spinner/>:
        !Array.isArray(teamList)||teamList.length===0?<EmptyState msg="No team WFH records"/>:
        teamList.map((wfh,idx)=>renderWFHCard(wfh,idx,null))
      )}
      {activeTab==="forwarded"&&(
        fwdLoading?<Spinner/>:
        !Array.isArray(forwardedList)||forwardedList.length===0?<EmptyState msg="No forwarded WFH requests"/>:
        forwardedList.map((wfh,idx)=>renderWFHCard(wfh,idx,fwdActions))
      )}
    </div>
  );
};
...
const ManagerLeaveWFH = () => {
  ...
  const manager = meData?.manager||meData;
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#F7F3FC 0%,#F0EBF8 50%,#F4F0FA 100%)",fontFamily:"'DM Sans',sans-serif",padding:"32px 36px"}}>
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:1100,margin:"0 auto"}}>
        ...
        {tab==="myBalance"      && <MyBalancePanel manager={manager}/>}
