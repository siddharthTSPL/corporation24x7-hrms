...
const STATUS_META = {
  ...
  forwarded_reporting_manager: {
    label: "Forwarded to Reporting Manager",
    bg: "#EFF6FF",
    color: "#1D4ED8",
    dot: "#3B82F6",
  },
  approved_reporting_manager: {
    label: "Approved by Reporting Manager",
    bg: "#F0FDF4",
    color: "#14803D",
    dot: "#22C55E",
  },
  rejected_reporting_manager: {
    label: "Rejected by Reporting Manager",
    bg: "#FEF2F2",
    color: "#991B1B",
    dot: "#EF4444",
  },
...
const decisionInfo = (leave) => {
  ...
  const name = person
    ...
    : "";
  const role = actorModel
    ? ROLE_LABEL[actorModel] || actorModel
    : leave.admin !== undefined
      ? "Super Admin"
      : "";
...
const ExportButton = ({ onClick, disabled }) => (
  ...
  >
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
      ...
      />
      <path
        d="M1.5 9.5v1a1 1 0 001 1h7a1 1 0 001-1v-1"
        stroke="currentColor"
        strokeWidth="1.4"
        strokeLinecap="round"
      />
...
const Toast = ({ toast }) => {
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 sm:bottom-8 sm:right-8 left-4 sm:left-auto px-4 py-3 sm:px-5 sm:py-3.5 rounded-xl text-[13px] font-medium shadow-lg z-[9999] flex items-center gap-2.5 transition-all duration-300 backdrop-blur-md ${c.bg} ${c.color} ${c.border} border`}
      style={{
...
const TypeBadge = ({ type }) => {
  const m = LEAVE_META[type] || {
    label: humanizeStatus(type || "other"),
    bg: "#F3F4F6",
    color: "#374151",
    dot: "#9CA3AF",
  };
...
const SummaryStrip = ({ stats }) => (
  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
    {stats.map((s, i) => (
      ...
      >
        <span
          className="text-2xl sm:text-3xl font-extrabold leading-none shrink-0"
          style={{ color: s.color, fontFamily: "'Playfair Display',serif" }}
        >
          {s.val}
        </span>
        <span
          className="text-[11px] sm:text-xs font-semibold leading-tight min-w-0"
...
const PersonCard = ({
  ...
  return (
    <div className="flex items-center gap-3 sm:gap-4 mb-3 min-w-0">
      <div
        className="w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 text-white text-sm font-bold shadow-md"
      ...
      <div className="min-w-0 flex-1">
        <div
          className={`text-sm font-semibold truncate ${hasPerson ? "text-gray-800" : "text-gray-400 italic"}`}
        >
        ...
        <div className="text-[11px] text-gray-400 mt-0.5 flex items-center gap-1.5 flex-wrap">
          <span className="truncate">
            {personEmail || (hasPerson ? "" : "Profile unavailable")}
          </span>
          {person?.designation && (
...
const ActionButtons = ({ onApprove, onReject }) => (
  <div className="flex sm:flex-col gap-2 w-full sm:w-auto shrink-0">
    <button
      className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold transition-all hover:-translate-y-0.5 bg-green-50 text-green-700 shadow-sm"
      onClick={onApprove}
    >
      <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
        <path
    ...
    <button
      className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold transition-all hover:-translate-y-0.5 bg-red-50 text-red-700 shadow-sm"
      onClick={onReject}
    >
      <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
        <path
...
const TerminalIcon = ({ status }) => {
  ...
  return (
    <div
      ...
      style={{ background: sm.bg, borderColor: sm.dot }}
    >
      {isApproved && (
...
const LeaveCard = ({
  ...
  return (
    <div
      className="bg-white rounded-2xl border border-purple-100 p-4 sm:p-5 mb-3.5 shadow-sm hover:shadow-md transition-all relative overflow-hidden animate-fadeSlideUp flex flex-col sm:flex-row justify-between items-start gap-4"
      style={{
    ...
    >
      <div
        className="absolute top-0 left-0 w-1 bottom-0 rounded-l-2xl"
      ...
      <div className="flex-1 min-w-0 w-full pl-2">
        ...
        {leave.reason && (
          <div className="bg-purple-50/50 rounded-xl px-3.5 py-2.5 text-xs text-gray-600 mt-2.5 border-l-4 border-purple-300 leading-relaxed">
            <span className="text-[#730042] font-semibold">Reason — </span>
            {leave.reason}
          </div>
        )}
        {leave.remarks && (
          <div className="bg-blue-50/50 rounded-xl px-3.5 py-2.5 text-xs text-blue-800 mt-2 border-l-4 border-blue-300 leading-relaxed">
            <span className="text-blue-600 font-semibold">Remarks — </span>
            {leave.remarks}
          </div>
        )}
        {leave.supportingDocument?.url && (
          <div className="bg-emerald-50/70 rounded-xl px-3.5 py-2.5 text-xs text-emerald-800 mt-2 border-l-4 border-emerald-300 leading-relaxed flex flex-wrap items-center justify-between gap-2">
            <div className="min-w-0">
              <span className="text-emerald-700 font-semibold">
                Supporting Document —{" "}
              </span>
              <span className="break-all">
                {leave.supportingDocument.originalName ||
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-[#730042] to-[#CD166E] text-white font-semibold hover:from-[#5a0033] hover:to-[#a81258] transition-colors shrink-0"
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                <path
                  d="M14 3h7v7M10 14L21 3M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
        ...
        {decisionInfo(leave) && (
          <div className="bg-rose-50/50 rounded-xl px-3.5 py-2.5 text-xs text-rose-800 mt-2 border-l-4 border-rose-300 leading-relaxed">
            <span className="text-rose-700 font-semibold">Decision — </span>
            {decisionInfo(leave)}
          </div>
        )}
      </div>
    ...
    >
      ...
      <div className="flex-1 min-w-0 w-full pl-2">
        ...
        <div className="flex items-center gap-1.5 text-xs text-gray-400 flex-wrap">
          ...
          <span className="font-medium text-gray-600">
            {formatDate(wfh.startDate)}
          </span>
          <span className="text-[10px]">→</span>
          <span className="font-medium text-gray-600">
            {formatDate(wfh.endDate)}
          </span>
        </div>
        {wfh.reason && (
          <div className="bg-blue-50/50 rounded-xl px-3.5 py-2.5 text-xs text-blue-800 mt-2.5 border-l-4 border-blue-300 leading-relaxed">
            <span className="text-blue-600 font-semibold">Reason — </span>
            {wfh.reason}
          </div>
        )}
        {wfh.remarks && (
          <div className="bg-purple-50/50 rounded-xl px-3.5 py-2.5 text-xs text-gray-600 mt-2 border-l-4 border-purple-300 leading-relaxed">
            <span className="text-[#730042] font-semibold">Remarks — </span>
            {wfh.remarks}
          </div>
        )}
      </div>
...
const EmployeeLeavesTab = ({ leaves, isLoading, processingId }) => {
  ...
  const handleExport = () => {
    exportLeavesToCSV(
      filtered,
      `employee-leaves-${new Date().toISOString().slice(0, 10)}.csv`,
      "employee",
    );
  };
        ...
        stats={[
          {
            ...
            val: leaves.length,
            color: "#730042",
            bg: "linear-gradient(135deg,#FFF0F7,#FFE4F2)",
          },
          {
            label: "Pending",
            val: leaves.filter(
              (l) => l.status === "forwarded_reporting_manager",
            ).length,
            color: "#1D4ED8",
          ...
          {
            label: "Approved",
            val: leaves.filter((l) => l.status === "approved_reporting_manager")
              .length,
            color: "#14803D",
          ...
          {
            label: "Rejected",
            val: leaves.filter((l) => l.status === "rejected_reporting_manager")
              .length,
            color: "#991B1B",
        ...
        {FILTERS.map((f) => {
          ...
          return (
            ...
            >
              {f.label}
              <span
                className={`px-1.5 py-0.5 rounded-md text-[10px] font-bold ${active ? "bg-white/25 text-white" : "bg-purple-50 text-gray-400"}`}
              >
                {count(f.key)}
              </span>
            </button>
...
const ManagerLeavesTab = ({ leaves, isLoading, processingId }) => {
  ...
  const handleExport = () => {
    exportLeavesToCSV(
      filtered,
      `manager-leaves-${new Date().toISOString().slice(0, 10)}.csv`,
      "manager",
    );
  };
  ...
  return (
    <div>
      <SummaryStrip
        stats={[
          ...
          },
          {
            ...
            val: leaves.filter(
              (l) =>
                l.status === "pending_reporting_manager" ||
          ...
          },
          {
            ...
            val: leaves.filter(
              (l) =>
                l.status === "approved_reporting_manager" ||
                l.status === "approved_admin",
            ).length,
            color: "#14803D",
            bg: "linear-gradient(135deg,#F0FDF4,#DCFCE7)",
          },
          {
            ...
            val: leaves.filter(
              (l) =>
                l.status === "rejected_reporting_manager" ||
      ...
      <div className="mb-5 bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-xl px-4 py-3 text-xs text-green-700 leading-relaxed flex gap-3 items-start">
        ...
        <span>
          <strong>Manager Leave Requests</strong> — Leave applications submitted
          by managers. These are handled by admins — shown here for your
          visibility.
        </span>
      </div>
...
const AdminLeavesTab = ({ leaves, isLoading, processingId, onAction }) => {
  ...
  const handleExport = () => {
    exportLeavesToCSV(
      filtered,
      `admin-leaves-${new Date().toISOString().slice(0, 10)}.csv`,
      "admin",
    );
  };
  ...
  return (
    <div>
      ...
      <div className="mb-5 bg-gradient-to-br from-pink-50 to-pink-100 border border-pink-200 rounded-xl px-4 py-3 text-xs text-[#730042] leading-relaxed flex gap-3 items-start">
        ...
        <span>
          <strong>Admin Leave Requests</strong> — Leave applications submitted
          by admins awaiting your final approval.
        </span>
      </div>
...
const WFHTab = ({ wfhList, isLoading, processingId, onAction }) => {
  ...
  const count = (key) =>
    key === "all"
      ? wfhList.length
      : wfhList.filter((w) => w.status === key).length;
  const statusFiltered =
    filter === "all" ? wfhList : wfhList.filter((w) => w.status === filter);
  ...
  const handleExport = () => {
    exportWFHToCSV(
      filtered,
      `wfh-requests-${new Date().toISOString().slice(0, 10)}`,
    );
  };
  ...
  return (
    <div>
      ...
      <div className="mb-5 bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl px-4 py-3 text-xs text-blue-700 leading-relaxed flex gap-3 items-start">
        ...
        <span>
          <strong>WFH Requests</strong> — Work-from-home requests from admins,
          managers and employees requiring your final approval.
        </span>
      </div>
...
const SuperAdminLeaveWFH = () => {
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-gradient-to-br from-purple-50 via-purple-100 to-purple-50 flex flex-col">
      ...
      {/* Background Blobs */}
      <div
        className="fixed top-[-80px] right-[-80px] w-80 h-80 rounded-full pointer-events-none z-0"
        style={{
          background:
            "radial-gradient(circle,rgba(205,22,110,0.07) 0%,transparent 70%)",
        }}
      />
      <div
        className="fixed bottom-[-60px] left-[-60px] w-72 h-72 rounded-full pointer-events-none z-0"
        style={{
          background:
            "radial-gradient(circle,rgba(115,0,66,0.06) 0%,transparent 70%)",
        }}
      />
        ...
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6 mb-6 sm:mb-8 w-full">
          <div className="flex items-center gap-3 sm:gap-4 w-full min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h1
                  className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 tracking-tight"
              ...
              <p className="text-xs text-gray-400 mt-1 hidden sm:block">
                Final approval authority · Leave requests & WFH across the
                organisation
              </p>
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-3 shrink-0">
            ...
            <div className="bg-white border border-purple-100 rounded-xl px-4 py-2 flex items-center gap-2.5 shadow-sm">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#730042] to-[#CD166E] text-white text-xs font-bold flex items-center justify-center">
                SA
              </div>
              <div>
          ...
          {TABS.map((t) => {
            ...
            return (
              ...
              >
                {t.label}
                <span
                  className={`px-1.5 py-0.5 rounded-md text-[10px] font-bold ${active ? "bg-white/25 text-white" : "bg-purple-100 text-gray-500"}`}
                >
                  {t.count}
                </span>
              </button>
