...
import {
  ...
  FaHistory, FaEllipsisV, FaUserTie, FaUser, FaCubes,
} from "react-icons/fa";
import {
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-xl shadow-2xl flex flex-col max-h-[94vh] animate-[modalUp_0.22s_ease-out]">
        <div className="sticky top-0 z-10 bg-white px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4 border-b border-[#e8d5e2] flex items-center justify-between rounded-t-2xl">
          <div>
            ...
            <p className="text-[11px] text-[#c499b4] mt-0.5">{isEdit ? "Update asset details" : "Register a new asset and its stock quantity"}</p>
          </div>
...
function AssignPersonModal({ open, onClose, asset, assignType, employees, managers, onAssign, loading }) {
  ...
  useEffect(() => { if (open) { setSelected(""); setQuantity("1"); } }, [open]);
  ...
  const handleAssign = () => {
    if (isEmployee) {
      onAssign({ id: asset._id, employee_id: selected, quantity: qtyNum });
    } else {
      onAssign({ id: asset._id, manager_id: selected, quantity: qtyNum });
    }
  };
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl animate-[modalUp_0.22s_ease-out]">
        ...
        <div className="px-4 sm:px-6 pb-4 sm:pb-5 flex gap-3">
          ...
          <button
            ...
            disabled={loading || !selected || !qtyValid}
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex" onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      <div className="w-full max-w-sm bg-white shadow-2xl flex flex-col border-l border-[#e8d5e2] h-full overflow-hidden">
        ...
        <div className="px-4 py-3 border-b border-[#e8d5e2] bg-[#F9F8F2]">
          <QuantityPill available={asset.available_quantity ?? 0} total={asset.total_quantity ?? 1} />
          <span className="text-[11px] text-[#993556] ml-2">units available / total</span>
        </div>
                  ...
                  return (
                    <div key={a._id} className="p-3 rounded-xl border border-[#e8d5e2] bg-[#fdf5f9] flex items-start justify-between gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-amber-300 text-amber-700 text-[11px] font-semibold hover:bg-amber-50 flex-shrink-0"
                      >
...
export default function AdminAssets() {
  ...
  const exportCsv = () => {
    const headers = [
      "Asset ID","Name","Type","Brand","Model Number","Serial Number","Condition",
      "Status","Total Quantity","Available Quantity","Assigned Units",
      "Purchase Date","Purchase Price","Notes",
      "Active Assignees","Assignment History",
    ];
    const escape = (v) => { const s = String(v ?? ""); return s.includes(",")||s.includes('"')||s.includes("\n") ? `"${s.replace(/"/g,'""')}"` : s; };
    const rows = rawAssets.map((a) => {
      const active = (a.assignments || []).filter((x) => !x.is_returned);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-4 sm:mb-6 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={exportCsv}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[#730042] text-[#730042] text-[13px] font-semibold hover:bg-[#f7ecf3] active:scale-95 transition min-h-[44px]"
            >
