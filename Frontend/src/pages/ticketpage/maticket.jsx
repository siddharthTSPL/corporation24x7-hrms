...
const GlobalStyles = () => (
  <style>{`
    ...
    .mgr-btn:hover  { transform:translateY(-1px); filter:brightness(1.05); }
...
const TypeChip = ({type}) => {
  ...
  return <span className="mgr-chip" style={{background:m.bg,color:m.color}}><span>{m.icon}</span>{m.label}</span>;
};

const StatusChip = ({status}) => {
  const m = STATUS_META[status]||{label:status,bg:"#F3F4F6",color:"#374151",dot:"#9CA3AF"};
  return <span className="mgr-chip" style={{background:m.bg,color:m.color}}><span style={{width:6,height:6,borderRadius:"50%",background:m.dot,display:"inline-block"}}/>{m.label}</span>;
};
    ...
    <div style={{background:"#fff",borderRadius:20,border:"1px solid rgba(180,155,210,.25)",padding:28,boxShadow:"0 4px 20px rgba(60,20,80,.07)"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
        <div>
          ...
          <select className="mgr-input" value={form.category} onChange={e=>set("category",e.target.value)}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
        <div>
          ...
          <input className="mgr-input" type="date" value={form.incidentDate} onChange={e=>set("incidentDate",e.target.value)}/>
        </div>
        <div>
      ...
      <button className="mgr-btn" onClick={handleSubmit} disabled={submitMut.isPending}
  ...
  return (
    <>
      {tickets.map((t,i)=>{
        ...
        return (
          <div key={t._id} className="mgr-card" style={{animationDelay:`${i*.05}s`,borderLeft:`3px solid ${tm.dot||"#730042"}`}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:7,alignItems:"center"}}>
                  ...
                  {t.isAnonymous&&<span className="mgr-chip" style={{background:"#F3F4F6",color:"#6B7280"}}>Anonymous</span>}
                </div>
