...
const TICKET_TYPES = {
  suggestion:    { label:"Suggestion",    icon:"💡", bg:"#DCFCE7", color:"#14803D", dot:"#22C55E" },
  complaint:     { label:"Complaint",     icon:"📋", bg:"#FEF3C7", color:"#92400E", dot:"#F59E0B" },
  posh:          { label:"POSH",          icon:"🔴", bg:"#FEE2E2", color:"#991B1B", dot:"#EF4444" },
  grievance:     { label:"Grievance",     icon:"⚖️", bg:"#EDE9FE", color:"#5B21B6", dot:"#8B5CF6" },
  whistleblower: { label:"Whistleblower", icon:"🔒", bg:"#DBEAFE", color:"#1D4ED8", dot:"#3B82F6" },
};
...
const GlobalStyles = () => (
  <style>{`
    ...
    .adm-input:focus { border-color:#730042!important; box-shadow:0 0 0 3px rgba(115,0,66,.09); }
    .adm-chip {
...
const Spinner = () => (
  <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"48px 0",flexDirection:"column",gap:12}}>
    ...
    <p style={{fontSize:12,color:C.muted,fontFamily:"'DM Sans',sans-serif"}}>Loading…</p>
  </div>
);

const Label = ({children,required}) => (
  <div style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:".5px",marginBottom:7,fontFamily:"'DM Sans',sans-serif"}}>
  ...
  return (
    <div style={{background:"#fff",borderRadius:20,border:"1px solid rgba(180,155,210,.25)",padding:28,boxShadow:"0 4px 20px rgba(60,20,80,.07)"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}>
        ...
        <div>
          <div style={{fontSize:17,fontWeight:700,color:C.text,fontFamily:"'Playfair Display',serif"}}>New Ticket</div>
          <div style={{fontSize:12,color:C.muted,fontFamily:"'DM Sans',sans-serif"}}>Your submission is confidential</div>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
        ...
        <div>
          ...
          <input className="adm-input" value={form.incidentLocation} onChange={e=>set("incidentLocation",e.target.value)} placeholder="Office floor, remote, etc."/>
        </div>
      </div>
            ...
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:7,alignItems:"center"}}>
                  <span style={{fontSize:10,fontWeight:700,color:C.muted,fontFamily:"'DM Sans',sans-serif"}}>{t.ticketNumber}</span>
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans',sans-serif",padding:"32px 36px"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:28}}>
        ...
        <div>
          <h1 style={{fontSize:22,fontWeight:700,color:C.text,margin:0,fontFamily:"'Playfair Display',serif"}}>My Tickets</h1>
          <p style={{fontSize:12,color:C.muted,margin:"3px 0 0"}}>Admin · Submit &amp; track your grievances, complaints, and suggestions</p>
        </div>
      </div>
        ...
        {[["submit","📝 Submit New"],["mytickets",`📋 My Tickets (${count})`]].map(([k,l])=>{
          ...
          return (
            <button key={k} onClick={()=>setTab(k)}
              style={{padding:"8px 20px",borderRadius:9,border:"none",cursor:"pointer",fontSize:12,fontWeight:active?600:400,fontFamily:"'DM Sans',sans-serif",color:active?"#fff":C.muted,background:active?`linear-gradient(135deg,${C.primary},${C.accent})`:"transparent",boxShadow:active?"0 2px 10px rgba(115,0,66,.28)":"none",transition:"all .2s"}}>
              {l}
            </button>
          );
        })}
