...
import { useEmployeeSubmitTicket, useEmployeeGetMyTickets, useEmployeeRateTicket, useGetEmployeeTicketDetail } from "../../auth/server-state/employee/employeeticket/employeeticket.hook";
...
const G = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .e-card:hover { box-shadow:0 6px 24px rgba(115,0,66,.12); border-color:rgba(115,0,66,.22); transform:translateY(-2px); }
    .e-btn {
    ...
    .e-btn:disabled { opacity:.45; cursor:not-allowed; transform:none!important; filter:none!important; }
    .e-input {
    ...
    .e-input:focus { border-color:#730042; box-shadow:0 0 0 3px rgba(115,0,66,.08); }
    .e-chip {
      display:inline-flex; align-items:center; gap:4px;
      padding:3px 9px; border-radius:20px;
      font-size:11px; font-weight:600; font-family:'Instrument Sans',sans-serif;
    }
    .e-detail { animation:slideIn .26s ease both; }
    .e-tab-active { background:#730042; color:#fff; box-shadow:0 2px 10px rgba(115,0,66,.3); }
    .e-tab-idle   { background:transparent; color:#A08CB8; }
    .e-tab-idle:hover { background:rgba(115,0,66,.06); color:#730042; }
    .e-skeleton {
...
const SkeletonDetail = () => (
  <div style={{display:"flex",flexDirection:"column",gap:12,padding:24}}>
    {[200,120,80,160,100].map((w,i)=>(
      <div key={i} className="e-skeleton" style={{height:16,width:`${w}px`,maxWidth:"100%"}}/>
    ))}
  </div>
);
...
const TypeChip = ({type}) => {
  ...
  return <span className="e-chip" style={{background:m.bg,color:m.color}}>{m.icon} {m.label}</span>;
};

const StatusChip = ({status}) => {
  const m = STATUS_META[status]||{label:status,bg:"#F3F4F6",color:"#374151",dot:"#9CA3AF"};
  return <span className="e-chip" style={{background:m.bg,color:m.color}}><span style={{width:5,height:5,borderRadius:"50%",background:m.dot,display:"inline-block"}}/>{m.label}</span>;
};

const SevChip = ({sev}) => {
  const m = SEV_META[sev]||{label:sev,bg:"#F3F4F6",color:"#374151"};
  return <span className="e-chip" style={{background:m.bg,color:m.color}}>{m.label}</span>;
};
    ...
    <div style={{background:"#fff",borderRadius:18,border:`1px solid ${C.border}`,padding:28,boxShadow:"0 4px 24px rgba(80,20,90,.06)"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
        <div>
          ...
          <select className="e-input" value={form.category} onChange={e=>set("category",e.target.value)}>
        ...
        <div>
          <Lbl>Severity</Lbl>
          <select className="e-input" value={form.severity} onChange={e=>set("severity",e.target.value)}>
      ...
      <div style={{marginBottom:16}}>
        <Lbl req>Title</Lbl>
        <input className="e-input" value={form.title} onChange={e=>set("title",e.target.value)} placeholder="Brief, clear title for your ticket…" maxLength={120}/>
      </div>

      <div style={{marginBottom:16}}>
        <Lbl req>Description</Lbl>
        <textarea className="e-input" rows={4} value={form.description} onChange={e=>set("description",e.target.value)} placeholder="Describe the issue in detail. Include dates, people involved, and what happened…" style={{resize:"vertical",lineHeight:1.7}}/>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
        <div>
          <Lbl>Incident Date</Lbl>
          <input className="e-input" type="date" value={form.incidentDate} onChange={e=>set("incidentDate",e.target.value)}/>
        </div>
        <div>
          <Lbl>Location</Lbl>
          <input className="e-input" value={form.incidentLocation} onChange={e=>set("incidentLocation",e.target.value)} placeholder="Office floor, remote, etc."/>
        </div>
      </div>

      <div style={{marginBottom:20}}>
        <Lbl>Witness Names <span style={{fontWeight:400,textTransform:"none",fontSize:10}}>(comma-separated)</span></Lbl>
        <input className="e-input" value={form.witnessNames} onChange={e=>set("witnessNames",e.target.value)} placeholder="John Doe, Jane Smith"/>
      </div>
      ...
      <button className="e-btn" onClick={submit} disabled={mut.isPending}
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(16,4,28,.55)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(6px)"}}>
      <div style={{background:"#fff",borderRadius:20,padding:28,width:380,boxShadow:"0 24px 64px rgba(0,0,0,.22)",animation:"fadeUp .24s ease both"}}>
        ...
        <textarea className="e-input" rows={3} value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="Optional feedback…" style={{marginBottom:18,resize:"none"}}/>
        <div style={{display:"flex",gap:10}}>
          <button className="e-btn" onClick={onClose} style={{background:C.surface,color:C.subtext,flex:1,justifyContent:"center",border:`1px solid ${C.border}`}}>Cancel</button>
          <button className="e-btn" onClick={submit} disabled={!rating||mut.isPending}
      ...
      <div style={{background:"#fff",borderRadius:18,border:`1px solid ${C.border}`,overflow:"hidden",boxShadow:"0 4px 24px rgba(80,20,90,.06)"}}>
        <div style={{background:`linear-gradient(130deg,${C.primary}12,${C.accent}08)`,padding:"22px 26px",borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10,alignItems:"center"}}>
                ...
                {ticket.isOverdue&&<span className="e-chip" style={{background:"#FEF2F2",color:"#991B1B",animation:"pulse 1.5s infinite"}}>⚠ Overdue</span>}
                {ticket.isEscalated&&<span className="e-chip" style={{background:"#FFF7ED",color:"#9A3412"}}>🔺 Escalated</span>}
              </div>
            ...
            {canRate&&(
              <button className="e-btn" onClick={()=>onRate(ticket)} style={{background:"linear-gradient(135deg,#D97706,#F59E0B)",color:"#fff",flexShrink:0}}>⭐ Rate Resolution</button>
            )}
          </div>
          ...
          <div style={{display:"flex",gap:3,marginTop:18,background:"rgba(255,255,255,.7)",borderRadius:10,padding:3,width:"fit-content",border:`1px solid ${C.border}`}}>
            {TABS.map(t=>(
              <button key={t.key} onClick={()=>setTab(t.key)} className={tab===t.key?"e-tab-active":"e-tab-idle"}
              ...
              {ticket.witnessNames?.length>0&&(
                <div style={{background:"#FFFBEB",borderRadius:12,padding:14,border:"1px solid rgba(245,158,11,.18)"}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {ticket.witnessNames.map((w,i)=><span key={i} className="e-chip" style={{background:"#FEF3C7",color:"#78350F"}}>👤 {w}</span>)}
                  </div>
                </div>
              )}
