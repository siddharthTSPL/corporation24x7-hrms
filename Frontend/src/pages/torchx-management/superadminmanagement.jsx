...
import {
  useGetAllDepartmentsSuperAdmin,
  useCreateDepartmentSuperAdmin,
...
const inputCls =
  'w-full min-w-0 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#730042]/30 focus:border-[#730042] transition-colors';
...
function Button({ children, onClick, variant = 'primary', className = '', disabled, type = 'button' }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`inline-flex items-center justify-center gap-1.5 rounded-lg px-3.5 py-2 min-h-[40px] text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${styles[variant]} ${className}`}
    >
      {children}
    </button>
  );
      ...
      {DAYS.map((d) => {
        ...
        return (
          <button
            ...
            className={`px-2.5 py-1.5 min-h-[36px] rounded-lg text-xs font-semibold border transition-colors ${
...
function EmptyRow({ text }) {
  return <div className="text-sm text-slate-400 text-center py-8 border border-dashed border-slate-200 rounded-xl px-3 break-words">{text}</div>;
}
...
function ShiftHistoryModal({ open, onClose, employeeId, role, personName, shifts, notify }) {
  const { data, isLoading } = useGetShiftHistorySuperAdmin(employeeId, role);
  const editMutation = useEditShiftAssignmentSuperAdmin();
  const deleteMutation = useDeleteShiftAssignmentSuperAdmin();
    ...
    <div className="flex flex-col gap-6 w-full min-w-0">
            ...
            {shifts.map((s) => (
              <div key={s._id} className="rounded-xl border border-slate-200 p-4 flex flex-col gap-3 hover:border-[#730042] transition-colors min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <h3 className="font-semibold text-slate-800 text-sm break-words">{s.name}</h3>
                      {s.isDefault && (
                ...
                <div className="flex flex-col sm:flex-row gap-2 pt-1">
                  {!s.isDefault && (
                    <Button variant="subtle" className="flex-1 text-xs py-1.5 min-h-0" onClick={() => makeDefault(s._id)}>
                      Set default
                    </Button>
                  )}
                  <Button variant="danger" className="flex-1 text-xs py-1.5 min-h-0" onClick={() => removeShift(s)}>
      ...
      <SectionCard icon={UserPlus} title="Assign shift to a person" subtitle="Choose who follows which shift; leave shift empty to use the org default">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
          ...
          <Button onClick={submitAssign} disabled={assignShift.isPending} className="w-full h-[38px]">
        ...
        <>
          <SectionCard icon={Users} title="Week-off groups" subtitle="Teams that can be given a different off-day in the same week">
            <div className="flex flex-col sm:flex-row gap-2 mb-4">
              ...
              <Button onClick={createGroupSubmit} disabled={createGroup.isPending} className="w-full sm:w-auto">
                ...
                {groups.map((g) => (
                  <div key={g._id} className="rounded-xl border border-slate-200 p-4 min-w-0">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      ...
                      />
                      <Button variant="subtle" className="py-1.5 px-2.5 w-full sm:w-auto min-h-0" onClick={() => addMember(g._id)}>
      ...
      <SectionCard icon={ShieldCheck} title="Individual overrides" subtitle="Give one person a different week-off rule than the rest of the org">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="flex flex-col gap-3 min-w-0">
            ...
            <Button onClick={submitOverride} disabled={setEmployeeOverride.isPending} className="self-start w-full sm:w-auto">
          ...
          <div className="flex flex-col gap-3 min-w-0">
            ...
            <Button variant="danger" onClick={submitRemoveOverride} disabled={removeEmployeeOverride.isPending} className="self-start w-full sm:w-auto">
              Remove override
            </Button>
          </div>
        </div>
        ...
        {policyQuery.isLoading ? (
          ...
          <>
            ...
            <div className="flex justify-end pt-5">
              <Button onClick={submit} disabled={locked || setLeavePolicy.isPending} className="w-full sm:w-auto">
...
function DepartmentsPanel({ notify }) {
  const { data: deptData, isLoading: loading } = useGetAllDepartmentsSuperAdmin();
  const departments = deptData?.departments || [];

  const createMutation = useCreateDepartmentSuperAdmin();
  const updateMutation = useUpdateDepartmentSuperAdmin();
  const deleteMutation = useDeleteDepartmentSuperAdmin();
  ...
  return (
    <div className="flex flex-col gap-5 sm:gap-6 min-w-0">
      <SectionCard
        icon={Building2}
        title="Departments"
        subtitle="Add custom departments — they show up instantly in onboarding and edit-department dropdowns"
        action={
          <Button onClick={openCreate} className="w-full sm:w-auto">
