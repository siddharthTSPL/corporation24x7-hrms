    ...
    <>
      <Card style={{ overflow: "hidden" }}>
        <div style={{
          ...
          {isRunning && <div style={{ width: 8, height: 8, borderRadius: "50%", background: "rgba(255,255,255,0.9)", animation: "timerPulse 1.4s ease-in-out infinite" }} />}
          <span style={{ color: isRunning ? "rgba(255,255,255,0.9)" : C.textMuted, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em" }}>
            {isRunning ? "Timer Running" : isPaused ? "Timer Paused" : "No Active Timer"}
          </span>
          {timer?.job?.title && <span style={{ marginLeft: "auto", color: isRunning ? "rgba(255,255,255,0.75)" : C.textMuted, fontSize: 12, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 130 }}>{timer.job.title}</span>}
        </div>
        <div style={{ padding: "20px 24px" }}>
          <div style={{ fontFamily: "'SF Mono', 'Fira Code', monospace", fontSize: 42, fontWeight: 800, color: isRunning ? C.brand : isPaused ? C.amber : C.border, letterSpacing: "0.04em", lineHeight: 1, marginBottom: 16, userSelect: "none" }}>{fmtSeconds(displaySecs)}</div>
          {!timer ? (
    ...
    <Card style={{ overflow: "hidden" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", borderBottom: `1px solid ${C.border}` }}>
        {days.map((d, i) => {
          ...
          return (
            <div key={iso} style={{ padding: "12px 8px 10px", borderRight: i < 6 ? `1px solid ${C.border}` : "none", background: isToday ? C.brandLight : "transparent", textAlign: "center" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", minHeight: 140 }}>
        {days.map((d, i) => {
          ...
          const isToday = iso === today;
          return (
            <div key={iso} style={{ borderRight: i < 6 ? `1px solid ${C.border}` : "none", background: isToday ? "rgba(115,0,66,0.02)" : "transparent", padding: "8px 6px", display: "flex", flexDirection: "column", gap: 4 }}>
              {logs.map((log) => (
                <div key={log._id} title={`${log.job?.title || "—"} · ${fmtDuration(log.duration_minutes)}`} style={{ background: log.billable ? C.greenLight : C.brandLight, borderLeft: `3px solid ${log.billable ? C.green : C.brand}`, border: `1px solid ${log.billable ? C.green + "25" : C.brand + "20"}`, borderRadius: 6, padding: "5px 7px" }}>
              ...
              <button
                ...
                style={{ marginTop: "auto", background: "none", border: `1.5px dashed ${C.border}`, borderRadius: 6, padding: "4px", cursor: "pointer", color: C.textMuted, fontSize: 11, fontWeight: 600, width: "100%", transition: "all 0.15s" }}
              ...
              {TABS.map((t) => (
                <button key={t.id} onClick={() => setTab(t.id)} style={{
                  ...
                  {t.id === "approvals" && approvals.length > 0 && (
                    <span style={{ position: "absolute", top: 4, right: 4, background: C.red, color: "#fff", borderRadius: "50%", fontSize: 8, fontWeight: 800, width: 14, height: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>{approvals.length}</span>
                  )}
                </button>
              ))}
            </nav>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <Card>
              <div style={{ padding: "16px 22px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>Team Workload</div>
                  <div style={{ fontSize: 11, color: C.textMuted }}>Weekly capacity per person</div>
                </div>
                ...
                {heatmap.length === 0 ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    <div style={{ display: "flex", gap: 6, paddingLeft: 44 }}>
                      {DAY_NAMES.map((d) => <div key={d} style={{ flex: 1, textAlign: "center", fontSize: 10, color: C.textMuted, fontWeight: 700 }}>{d}</div>)}
                    </div>
                    {heatmap.map((row, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          {dayKeys.map((dk) => {
                            ...
                            return (
                              <div key={dk} title={`${pct}%`} style={{ flex: 1, height: 32, background: bg, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: col }}>
                                {pct > 0 ? `${pct}%` : "—"}
                              </div>
                            );
                          })}
                ...
                {overrunJobs.map((j) => (
                  <div key={j._id} style={{ padding: "12px 22px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: C.textMuted }}>{j.logged_hours_cache}h logged / {j.estimated_hours}h estimated</div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
              <div>
                ...
                <p style={{ fontSize: 13, color: C.textMuted, margin: "4px 0 0" }}>{createdJobs.length} jobs assigned to your team</p>
              </div>
              <Btn onClick={() => setJobModal(true)}>+ Create Job</Btn>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 20 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <Card style={{ padding: "14px 16px" }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 10 }}>My Assigned Jobs</div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <Card style={{ padding: "16px 18px" }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Total This Week</div>
                    <div style={{ fontSize: 22, fontWeight: 900, color: C.brand }}>{fmtDuration(totalWeekMins)}</div>
                  </Card>
                  <Card style={{ padding: "16px 18px" }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Capacity</div>
                    <div style={{ fontSize: 22, fontWeight: 900, color: C.green }}>{Math.round((totalWeekMins / 2400) * 100)}%</div>
                  </Card>
                </div>
                <CalendarWeekGrid
                  weekStart={weekStart}
        ...
        {tab === "approvals" && (
          <div>
            <div style={{ marginBottom: 20 }}>
              <h1 style={{ fontSize: 20, fontWeight: 800, color: C.text, margin: 0 }}>Pending Approvals</h1>
              <p style={{ fontSize: 13, color: C.textMuted, margin: "4px 0 0" }}>{approvals.length} timesheet{approvals.length !== 1 ? "s" : ""} in your queue</p>
            </div>
            {approvals.length === 0 ? (
              <Card style={{ padding: "60px 32px", textAlign: "center" }}>
                <div style={{ fontSize: 36, marginBottom: 12 }}>✅</div>
