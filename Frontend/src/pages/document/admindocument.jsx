...
import {
  ...
  useUpdateDocument,
  useDeleteDocument,
} from "../../auth/server-state/admindocument/admindocument.hook";
...
export default function AdminDocument() {
  ...
  const editMut   = useUpdateDocument();
...
const openEdit = (doc) => {
  ...
  const inputCls =
    "w-full px-3 py-2.5 text-[13px] border border-[rgba(115,0,66,0.12)] rounded-lg bg-[#F9F8F2] outline-none text-[#1A0A12] focus:border-[#730042] transition-colors box-border font-[inherit]";
  const labelCls =
    "block text-[11px] font-semibold text-[#730042] mb-1.5 uppercase tracking-wide";

  if (!canUpload && docs.length === 0 && !isLoading) return <NoPermission />;
  ...
  return (
    <div className="py-5 px-0 font-[inherit]">
            ...
            >
              <div
                className={`w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center ${
                  isPdf(doc.fileUrl)
                    ? "bg-[rgba(205,22,110,0.1)]"
                    : "bg-[rgba(115,0,66,0.08)]"
                }`}
              >
                {isPdf(doc.fileUrl) ? (
                  ...
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${
                      doc.fileType === "personal"
                        ? "bg-[rgba(205,22,110,0.1)] text-[#CD166E]"
                        : "bg-[rgba(115,0,66,0.08)] text-[#730042]"
                    }`}
                  >
                    {doc.fileType}
                  </span>
                  {doc.size && (
                    <span className="text-[11px] text-[#9B7A8E]">{doc.size} KB</span>
                  )}
      ...
      {toast && (
        <div
          className={`fixed bottom-6 left-1/2 -translate-x-1/2 px-5 py-2.5 rounded-xl text-[13px] font-medium border z-[300] shadow-lg whitespace-nowrap ${
            toast.kind === "ok"
