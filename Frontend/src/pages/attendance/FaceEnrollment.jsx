    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
      <div className="bg-white rounded-3xl p-6 w-full max-w-sm flex flex-col gap-4 shadow-2xl">
        ...
        <div className="relative w-full aspect-square rounded-2xl overflow-hidden bg-gray-100">
          <video ref={videoRef} className={`w-full h-full object-cover ${!snapshotUrl ? "block" : "hidden"}`} style={{ transform: "scaleX(-1)" }} playsInline muted />
        ...
        <div className="flex gap-2">
          ...
          {!snapshotUrl ? (
            <button onClick={takePhoto} disabled={!ready} className="flex-1 text-white rounded-xl py-2.5 text-sm font-bold disabled:opacity-50" style={{ background: MAROON }}>
              📷 Capture
            </button>
          ) : (
            <>
              ...
              <button onClick={() => onCapture(blob)} className="flex-1 text-white rounded-xl py-2.5 text-sm font-bold" style={{ background: MAROON }}>
                Use photo
              </button>
            </>
          )}
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          {filtered.map((emp) => {
            ...
            return (
              <div key={emp._id} className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 last:border-b-0">
                <div>
                  <p className="text-sm font-semibold text-gray-900">{emp.f_name} {emp.l_name}
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${isEnrolled ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                  ...
                  {isEnrolled ? (
                    ...
                    <button
                      ...
                      className="text-xs font-bold text-white rounded-lg px-3 py-1.5 disabled:opacity-50"
          ...
          {filtered.length === 0 && <p className="text-sm text-gray-400 px-5 py-6 text-center">No employees match your search.</p>}
        </div>
      )}
