...
const EmptyState = ({ title, subtitle }) => (
  <div className="flex flex-col items-center justify-center text-center py-14 px-4">
    <div className="w-16 h-16 rounded-full bg-[#730042]/10 flex items-center justify-center mb-4">
      <FaFolderOpen size={26} className="text-[#730042]/50" />
    </div>
...
const AllDocumentsSection = () => {
  ...
  const { data: personalData, isLoading: personalLoading } = useGetPersonalDocuments();
  const { data: expenseData, isLoading: expenseLoading } = useGetExpenseDocuments();

  const isLoading = subTab === SUB.PERSONAL ? personalLoading : expenseLoading;
  const docs =
    (subTab === SUB.PERSONAL ? personalData?.documents : expenseData?.documents) ?? [];
        ...
        <div className="hidden md:block">
          {isLoading ? (
            ...
            <EmptyState
              ...
              subtitle={`You have not uploaded any ${subTab} documents yet.`}
            />
          ) : (
            <table className="w-full text-sm">
              <thead>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {/* <div className="flex gap-6 border-b border-gray-200 mb-6">
          {[
            ...
            { key: TABS.ALL, label: "All Documents" },
          ].map((t) => (
            <button
              key={t.key}
