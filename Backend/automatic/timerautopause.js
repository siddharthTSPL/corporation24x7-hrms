...
const autoPauseStaleTimers = async () => {
  try {
    ...
    for (const timer of staleTimers) {
      const elapsedSeconds = Math.floor((timer.last_heartbeat_at - timer.started_at) / 1000);
      timer.accumulated_seconds = Math.max(timer.accumulated_seconds, elapsedSeconds);
      timer.status = "paused";
