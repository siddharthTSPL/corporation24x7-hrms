...
const timelineEntrySchema = new mongoose.Schema(
  {
    ...
    byModel:    { type: String, enum: ["SuperAdmin", "Admin", "Manager", "User", "System"] },
    byName:     String,          
...
const ticketSchema = new mongoose.Schema(
  {
    ...
    internalNotes:     [{ note: String, addedBy: mongoose.Schema.Types.ObjectId, addedAt: { type: Date, default: Date.now }, byName: String }],
...
ticketSchema.virtual("isOpen").get(function () {
  return ["open", "acknowledged", "under_review", "action_taken", "reopened"].includes(this.status);
});
...
ticketSchema.statics.getDashboardStats = async function () {
  const [typeCounts, statusCounts, overdueCounts, recentCritical, monthlyCounts] =
    await Promise.all([
