...
const resolveFirstHandler = async ({ actor, organisation_id }) => {
  if (actor.model === "User") {
    const user = await User.findOne({ _id: actor.id, organisation_id }).select("Under_manager").lean();
    if (!user?.Under_manager) return null;
    return { handler: user.Under_manager, handlerModel: "Manager", status: "pending_manager" };
  }
  ...
  if (actor.model === "Manager") {
    ...
    return {
      ...
      status: manager.reporting_manager_model === "Admin" ? "pending_admin" : "pending_reporting_manager",
    };
  }
  ...
  if (actor.model === "Admin") {
    ...
    return { handler: admin.reporting_manager, handlerModel: "SuperAdmin", status: "pending_superadmin" };
  }
...
const submitTimesheet = async (req, res, next) => {
  ...
  if (!logs.length) return next(httpError("No draft time logs found for this week", 400));
  ...
  const totalBilledAmount = logs.reduce((sum, l) => sum + (l.billed_amount || 0), 0);

  const routing = await resolveFirstHandler({ actor, organisation_id });

  let timesheet = await Timesheet.findOne({ organisation_id, owner: actor.id, week_start: start });
...
const getMyTimesheets = async (req, res, next) => {
  ...
  const timesheets = await Timesheet.find({ organisation_id, owner: actor.id, owner_model: actor.model })
    .sort({ week_start: -1 })
    .lean();
...
const getPendingApprovals = async (req, res, next) => {
  ...
  const timesheets = await Timesheet.find({
    ...
    status: { $in: ["pending_manager", "pending_reporting_manager", "pending_admin", "pending_superadmin"] },
  })
...
const approveTimesheet = async (req, res, next) => {
  ...
  const isHandler = timesheet.currentHandler?.toString() === actor.id.toString() && timesheet.currentHandlerModel === actor.model;
...
const rejectTimesheet = async (req, res, next) => {
  ...
  if (!timesheetId) return next(httpError("timesheetId is required", 400));
  if (!remarks) return next(httpError("remarks are required when rejecting a timesheet", 400));
  ...
  await timesheet.save();
  await TimeLog.updateMany({ timesheet: timesheet._id }, { $set: { status: "draft", timesheet: null } });
...
const forwardTimesheet = async (req, res, next) => {
  ...
  if (!timesheetId) return next(httpError("timesheetId is required", 400));
  if (actor.model !== "Manager") return next(httpError("Only managers can forward timesheets", 403));

  const timesheet = await Timesheet.findOne({ _id: timesheetId, organisation_id });
  if (!timesheet) return next(httpError("Timesheet not found", 404));

  const isHandler = timesheet.currentHandler?.toString() === actor.id.toString() && timesheet.currentHandlerModel === "Manager";
  if (!isHandler) return next(httpError("This timesheet is not in your queue", 403));
