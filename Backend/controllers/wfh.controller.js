...
const applyWFH = async (req, res, next) => {
  ...
  if (!employee.Under_manager)
    return next(
      Object.assign(new Error("No manager assigned. Cannot apply WFH."), { statusCode: 400 })
    );
...
const editWFH = async (req, res, next) => {
  ...
  if (wfh.status !== "pending_manager")
    return next(
      Object.assign(new Error("Cannot edit a WFH request that is already processed or forwarded"), { statusCode: 400 })
    );
...
const deleteWFH = async (req, res, next) => {
  ...
  if (wfh.status !== "pending_manager")
    return next(
      Object.assign(new Error("Cannot delete a WFH request that is already processed"), { statusCode: 400 })
    );
...
const getMyWFH = async (req, res, next) => {
  const wfhList = await WFH.find({ requester: req.employee._id })
...
const getAllTeamWFH = async (req, res, next) => {
  const wfhList = await WFH.find({ manager: req.manager._id })
    .populate("requester", "f_name l_name work_email department designation")
    .sort({ createdAt: -1 })
    .lean();
...
const approveWFH = async (req, res, next) => {
  ...
  if (wfh.manager.toString() !== req.manager._id.toString())
    return next(
      Object.assign(new Error("This WFH request does not belong to your team"), { statusCode: 403 })
    );
...
const forwardWFHToReportingManager = async (req, res, next) => {
  ...
  if (
    wfh.status.startsWith("approved") ||
    wfh.status.startsWith("rejected") ||
    wfh.status === "forwarded_reporting_manager"
  )
    return next(
      Object.assign(new Error("WFH request is already processed or forwarded"), { statusCode: 400 })
    );

  const currentManager = await Manager.findById(req.manager._id)
    .select("reporting_manager")
    .lean();

  if (!currentManager.reporting_manager)
    return next(
      Object.assign(new Error("You have no reporting manager assigned. Cannot forward WFH."), { statusCode: 400 })
    );
...
const managerApplyWFH = async (req, res, next) => {
  ...
  if (!startDate || !endDate || !reason)
    return next(
      Object.assign(
        new Error("startDate, endDate, and reason are required"),
        { statusCode: 400 }
      )
    );
  ...
  if (!currentManager.reporting_manager)
    return next(
      Object.assign(
        new Error("No reporting manager assigned. Cannot apply WFH."),
        { statusCode: 400 }
      )
    );
  ...
  const overlapping = await WFH.findOne({
    requester: managerId,
    status: {
      $nin: ["rejected_manager", "rejected_reporting_manager"],
    },
    startDate: { $lte: end },
    endDate: { $gte: start },
  })
  ...
  if (overlapping)
    return next(
      Object.assign(
        new Error("You already have a WFH request for overlapping dates"),
        { statusCode: 409 }
      )
    );
  ...
  res.status(201).json({
    success: true,
    message: "WFH request submitted to reporting manager",
    wfh,
  });
};

const managerGetMyWFH = async (req, res, next) => {
  const wfhList = await WFH.find({ requester: req.manager._id, requesterModel: "Manager" })
    .populate("manager", "f_name l_name work_email designation")
    .sort({ createdAt: -1 })
    .lean();
...
const getForwardedWFH = async (req, res, next) => {
  const wfhList = await WFH.find({
    manager: req.manager._id,
    status: { $in: ["forwarded_reporting_manager", "pending_reporting_manager"] },
  })
...
const approveForwardedWFH = async (req, res, next) => {
  ...
  if (
    ...
    return next(
      Object.assign(new Error("WFH request is not awaiting your approval"), { statusCode: 400 })
    );
...
const rejectForwardedWFH = async (req, res, next) => {
  ...
  if (
    ...
    return next(
      Object.assign(new Error("WFH request is not awaiting your decision"), { statusCode: 400 })
    );
...
const adminApplyWFH = async (req, res, next) => {
  ...
  if (!admin.organisation_id)
    return next(
      Object.assign(new Error("No organisation / superadmin assigned. Cannot apply WFH."), { statusCode: 400 })
    );
  ...
  const wfh = await WFH.create({
    ...
    superadmin: admin.organisation_id,
    startDate: start,
    endDate: end,
    days,
    reason,
...
const adminGetMyWFH = async (req, res, next) => {
  ...
  const wfhList = await WFH.find({ requester: req.admin._id, requesterModel: "Admin" })
