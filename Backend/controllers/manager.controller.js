...
const acceptleaverequest = async (req, res, next) => {
  ...
  if (leave.status !== "pending_manager")
    return next(Object.assign(new Error("Leave already processed or forwarded"), { statusCode: 400 }));
...
const forwardedtoreportingmanager = async (req, res, next) => {
  try {
    ...
    const currentManager = await managermodel.findById(req.manager._id)
...
const forwardEmployeeLeaveUpChain = async (req, res, next) => {
  if (!req.manager)
    return next(Object.assign(new Error("Unauthorized"), { statusCode: 401 }));
