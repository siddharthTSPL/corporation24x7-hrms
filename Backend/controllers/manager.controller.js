...
const managerlogin = async (req, res, next) => {
  ...
  if (!superAdmin) {
    const admin = await AdminModel.findById(organisationId).select("organisation_id").lean();
...
const forwardedtoreportingmanager = async (req, res, next) => {
  try {
    ...
    if (leave.status === "forwarded_reporting_manager")
      return next(Object.assign(new Error("Leave already forwarded"), { statusCode: 400 }));
...
const getforwardedleaves = async (req, res, next) => {
  ...
  const [employeeLeaves, managerLeaves] = await Promise.all([
    leavemodel
      .find({ organisation_id, status: "forwarded_reporting_manager", manager: { $in: subordinateManagerIds } })
      .populate("employee", "f_name l_name work_email department")
      .populate("manager", "f_name l_name work_email")
      .sort({ createdAt: -1 })
      .lean(),
    managerLeaveModel
      .find({ organisation_id, status: "pending_reporting_manager", manager: { $in: subordinateManagerIds } })
...
const acceptforwardedleave = async (req, res, next) => {
  ...
  if (!leaveId || !leaveFor)
    return next(Object.assign(new Error("leaveId and leaveFor (employee/manager) are required"), { statusCode: 400 }));
  ...
  if (leaveFor === "employee" && leave.leaveType === "ml") {
    const leaveBalance = await LeaveBalance.findOne({ employee: leave.employee, organisation_id });
    if (leaveBalance) {
      const start = new Date(leave.startDate);
      const end = new Date(start);
      end.setDate(end.getDate() + 181);
...
const applyleavem = async (req, res, next) => {
  ...
  const leave = await managerLeaveModel.create({
    organisation_id, manager: managerId, leaveType, startDate: start, endDate: end, days, reason,
...
const verifyManagerOtp = async (req, res, next) => {
  ...
  await Promise.all([
    sendEmail({
      ...
      html: `<h2>Hello ${manager.f_name}</h2><p>Your OTP verification was successful.</p><p>If you want to change your password, click the link below:</p><a href="${link}">Change Password</a>`,
    }),
    OtpModel.deleteOne({ email: work_email }),
  ]);
  res.status(200).json({ message: "OTP verified. Login successful.", role: manager.role, token, passwordResetOptional: true });
};

const showPasswordPageotp = (req, res) => {
  const token = req.query.token;
  res.send(
    `<h2>Set Your Password</h2><form action="/manager/resetManagerPassword" method="POST"><input type="hidden" name="token" value="${token}"/><input type="password" name="newPassword" placeholder="Enter new password" required/><button type="submit">Update Password</button></form>`,
  );
};
  ...
  try {
    ...
    if (gender !== undefined) {
      if (!["male", "female"].includes(gender))
        return next(Object.assign(new Error("Invalid gender"), { statusCode: 400 }));
    ...
    if (marital_status !== undefined) {
      if (!["single", "married", "divorced"].includes(marital_status))
        return next(Object.assign(new Error("Invalid marital status"), { statusCode: 400 }));
...
const managerSubmitTicket = async (req, res, next) => {
  try {
    ...
    const ticket = await Ticket.create({
      organisation_id: req.manager.organisation_id, type, category, subCategory, title, description,
      incidentDate, incidentLocation, witnessNames: witnessNames || [], severity: severity || "medium",
...
const getOrgInfoForManager = async (req, res, next) => {
  try {
    ...
    let superAdmin = await SuperAdminModel.findById(organisation_id)
