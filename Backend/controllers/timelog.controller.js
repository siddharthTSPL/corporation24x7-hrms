...
const recomputeJobHours = async (jobId) => {
  ...
  const job = await TSJob.findById(jobId);
  if (!job) return;

  job.logged_hours_cache = Math.round(totalHours * 100) / 100;

  if (job.estimated_hours && job.logged_hours_cache > job.estimated_hours && !job.overrun_flagged) {
...
const logTime = async (req, res, next) => {
  ...
  if (!job || !log_date || !duration_minutes) {
    return next(httpError("job, log_date, and duration_minutes are required", 400));
  }

  if (duration_minutes <= 0 || duration_minutes > 1440) {
    return next(httpError("duration_minutes must be between 1 and 1440", 400));
  }
  ...
  if (!isAssignee) return next(httpError("You can only log time against jobs assigned to you", 403));

  const isBillable = billable !== undefined ? !!billable : jobDoc.billable;
...
const getMyWeekLog = async (req, res, next) => {
  ...
  for (let i = 0; i < 7; i += 1) {
    ...
    dayBuckets[day.toISOString().slice(0, 10)] = { totalMinutes: 0, logs: [] };
  }

  for (const log of logs) {
    const key = new Date(log.log_date).toISOString().slice(0, 10);
    if (!dayBuckets[key]) dayBuckets[key] = { totalMinutes: 0, logs: [] };
    dayBuckets[key].totalMinutes += log.duration_minutes;
    dayBuckets[key].logs.push(log);
  }

  const totalMinutes = logs.reduce((sum, l) => sum + l.duration_minutes, 0);

  res.status(200).json({ success: true, week_start: start, totalMinutes, days: dayBuckets });
};
...
const updateTimeLog = async (req, res, next) => {
  ...
  const isOwner = timeLog.logged_by.toString() === actor.id.toString() && timeLog.logged_by_model === actor.model;
  if (!isOwner) return next(httpError("You can only edit your own time logs", 403));

  if (timeLog.status === "approved") {
    return next(httpError("Cannot edit a time log that has already been approved", 400));
  }

  if (duration_minutes !== undefined && duration_minutes !== timeLog.duration_minutes) {
    if (duration_minutes <= 0 || duration_minutes > 1440) {
      return next(httpError("duration_minutes must be between 1 and 1440", 400));
    }
  ...
  res.status(200).json({ success: true, message: "Time log updated", timeLog });
};
...
const deleteTimeLog = async (req, res, next) => {
  ...
  const isOwner = timeLog.logged_by.toString() === actor.id.toString() && timeLog.logged_by_model === actor.model;
  if (!isOwner) return next(httpError("You can only delete your own time logs", 403));

  if (timeLog.status !== "draft") {
    return next(httpError("Cannot delete a time log that has been submitted", 400));
  }
